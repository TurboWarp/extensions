    ...
    class TurboModz {
      ...
      isSprite(url) {
        try {
            ...
            if (parsedUrl.protocol === "data:" && parsedUrl.pathname.startsWith("application/x.scratch.sprite3;")) {
                return true;
            }
    ...
    isImage(url) { 
        ...
        try {
            ...
            if (parsedUrl.protocol === "data:" && url.startsWith("data:image/")) {
                return validFormats.some(format => url.startsWith(`data:image/${format};`));
            }
    ...
    isSound(url) {
        try {
            ...
            if (parsedUrl.protocol === "data:" && url.startsWith("data:audio/")) {
                return validFormats.some(format => url.startsWith(`data:audio/${format};`));
            }
      ...
      newMod(args) {
        if (!mods.some(mod => args.NAME === mod.name)){
            mods.push({name: args.NAME, id: newID(7), sprites: [], costumes: [], sounds: [], runtime_values:[]})
            console.log(mods)
        }
        else {
