...
(Scratch => {
  ...
  const GameJolt = (() => {
     ...
     */
    GJAPI.SendRequestEx = (
      sURL, bSendUser, sFormat, sBodyData,
      pCallback
    ) => {
    ...
    GJAPI.SessionOpen = pCallback => {
      ...
      if (GJAPI.iSessionHandle) { pCallback(); return; }
     ...
     */
    GJAPI.ScoreAdd = (
      iScoreTableID, iScoreValue, sScoreText, sExtraData,
      pCallback
    ) => {
      if (!GJAPI.bLoggedIn) {
        GJAPI.LogTrace(
     ...
     */
    GJAPI.ScoreAddGuest = (
      iScoreTableID, iScoreValue, sScoreText, sGuestName, sExtraData,
      pCallback
    ) => {
    ...
    GJAPI.TrophyRemove = (iTrophyID, pCallback) => {
      ...
      GJAPI.SendRequest(
        "/trophies/remove-achieved/?game_id=" + GJAPI.iGameID +
        "&username=" + GJAPI.sUserName +
        "&user_token=" + GJAPI.sUserToken +
        "&trophy_id=" + iTrophyID,
        GJAPI.SEND_FOR_USER,
    ...
    GJAPI.ScoreGetRank = (iScoreTableID, iScoreValue, pCallback) => {
      GJAPI.SendRequest(
        "/scores/get-rank/?game_id=" + GJAPI.iGameID +
        "&sort=" + iScoreValue +
        "&table_id=" + iScoreTableID,
        GJAPI.SEND_GENERAL,
        pCallback
      );
    };
    ...
    GJAPI.SessionCheck = pCallback => {
      GJAPI.SendRequest(
        "/sessions/check/?game_id=" + GJAPI.iGameID +
        "&username=" + GJAPI.sUserName +
        "&user_token=" + GJAPI.sUserToken,
        GJAPI.SEND_GENERAL,
        pCallback
    ...
    GJAPI.SessionSetStatus = (bIsOpen, pCallback) => {
      if (!GJAPI.bLoggedIn) {
        GJAPI.LogTrace("SessionSetStatus(" + bIsOpen + ") failed: no user logged in");
        pCallback(GJAPI.err.get("noLogin"));
        return;
      }
    ...
    GJAPI.UserFetchComb = (bIsUsername, sValue, pCallback) => {
      ...
      GJAPI.SendRequest("/users/?user_id=" + sValue, GJAPI.SEND_GENERAL, pCallback);
    };
     ...
     */
    GJAPI.ScoreFetchEx = (
      iScoreTableID, bOnlyUser, iLimit, bBetterOrWorse, iValue,
      pCallback
    ) => {
      if (!GJAPI.bLoggedIn && bOnlyUser) {
        GJAPI.LogTrace(
    ...
    GJAPI.ScoreFetchGuestEx = (
      iScoreTableID, sName, iLimit, bBetterOrWorse, iValue,
      pCallback
    ) => {
      GJAPI.SendRequest(
        "/scores/?limit=" + iLimit +
        (iScoreTableID ? "&table_id=" + iScoreTableID : "") +
        "&guest=" + sName +
        (bBetterOrWorse ? "&better_than=" : "&worse_than=") + iValue,
        GJAPI.SEND_GENERAL,
        pCallback
      );
    };
    ...
    GJAPI.TrophyFetchComb = (bIsAll, iValue, pCallback) => {
      if (!GJAPI.bLoggedIn) {
        GJAPI.LogTrace("TrophyFetchComb(" + bIsAll + ", " + iValue + ") failed: no user logged in");
        pCallback(GJAPI.err.get("noLogin"));
        return;
      }
      if (bIsAll) {
    ...
    GJAPI.UserLoginAuto = pCallback => {
      ...
      if (GJAPI.bLoggedIn) {
        GJAPI.LogTrace("UserLoginAuto() failed: user " + GJAPI.sUserName + " already logged in");
        pCallback(GJAPI.err.get("login"));
        return;
      }
    ...
    GJAPI.DataStoreGetKeysEx = (iStore, sPattern, pCallback) => {
      GJAPI.SendRequest('/data-store/get-keys/?pattern=' + sPattern, iStore, pCallback);
    };
  ...
  const docs = 'https://softedco.github.io/GameJolt-API-Scratch-extension/DOCUMENTATION';
  ...
  class GameJoltAPI {
    ...
    setGame({ID, key}) {
      GameJolt.iGameID = ID; GameJolt.sGameKey = key;
    }
    session({openOrClose}) {
      return new Promise(resolve => GameJolt.SessionSetStatus(openOrClose, resolve));
    }
    ...
    sessionBool() {
      return new Promise(resolve => GameJolt.SessionCheck(pResponse => resolve(pResponse.success == bool.t)));
    }
    ...
    loginManual({username, token}) {
      return new Promise(resolve => GameJolt.UserLoginManual(username, token, resolve));
    }
    ...
    userFetch({fetchType, usernameOrID}) {
      return new Promise(resolve => GameJolt.UserFetchComb(
        fetchType, usernameOrID,
        pResponse => resolve(
    ...
    userFetchCurrent() {
      return new Promise(resolve => GameJolt.UserFetchCurrent(
        pResponse => resolve(
          pResponse.success == bool.t ?
            data.user = pResponse.users[0] :
            err.user = pResponse.message
    ...
    trophyFetch({indexOrID, value, trophyDataType}) {
      ...
      GameJolt.TrophyFetchComb(
        indexOrID, indexOrID ? GameJolt.TROPHY_ALL : value,
        pResponse =>
      {
        if (pResponse.success == bool.f) { err.trophies = pResponse.message; return; }
        data.trophies = indexOrID ? pResponse.trophies : pResponse.trophies[0];
      });
    ...
    scoreAdd({ID, value, text, extraData}) {
      return new Promise(resolve => GameJolt.ScoreAdd(
        ID, value, text, extraData,
        resolve
      ));
    }
    scoreAddGuest({ID, value, text, username, extraData}) {
      return new Promise(resolve => GameJolt.ScoreAddGuest(
        ID, value, text, username, extraData,
        resolve
      ));
    }
    scoreFetch({globalOrPerUser, ID, amount, betterOrWorse, value}) {
      if (globalOrPerUser == bool.t && !GameJolt.bLoggedIn) { err.scores = err.noLogin; return; }
      return new Promise(resolve => GameJolt.ScoreFetchEx(
        ID, globalOrPerUser == bool.t ?
        GameJolt.SCORE_ONLY_USER : GameJolt.SCORE_ALL,
        amount, betterOrWorse, value,
        pResponse => resolve(pResponse.success == bool.t ?
          data.scores = pResponse.scores :
          err.scores = pResponse.message
      )));
    }
    scoreFetchGuest({ID, username, amount, betterOrWorse, value}) {
      return new Promise(resolve => GameJolt.ScoreFetchGuestEx(
        ID, username, amount, betterOrWorse, value,
        pResponse => resolve(pResponse.success == bool.t ?
          data.scores = pResponse.scores :
          err.scores = pResponse.message
      )));
    ...
    returnScoreData({index, scoreDataType}) {
      ...
      if (scoreDataType == 'user')
        return data.scores[index].user || data.scores[index].guest || err.get('noItem');
      return data.scores[index][scoreDataType] || err.get('noItem');
    }
    scoreGetRank({ID, value}) {
      return new Promise(resolve => GameJolt.ScoreGetRank(
        ID, value,
        pResponse => resolve(pResponse.success == bool.t ?
          pResponse.rank :
    ...
    dataStoreSet({globalOrPerUser, key, data}) {
      return new Promise(resolve => GameJolt.DataStoreSet(
        globalOrPerUser == bool.t, key, data,
        resolve
      ));
    }
    dataStoreFetch({globalOrPerUser, key}) {
      if (globalOrPerUser == bool.t && !GameJolt.bLoggedIn) return err.get('noLogin');
      return new Promise(resolve => GameJolt.DataStoreFetch(
        globalOrPerUser == bool.t, key,
        pResponse => resolve(pResponse.success == bool.t ?
          pResponse.data :
          err.show(pResponse.message)
      )));
    ...
    dataStoreUpdate({globalOrPerUser, key, operationType, value}) {
      return new Promise(resolve => GameJolt.DataStoreUpdate(
        globalOrPerUser == bool.t, key, operationType, value,
        resolve
      ));
    }
    dataStoreRemove({globalOrPerUser, key}) {
      return new Promise(resolve => GameJolt.DataStoreRemove(
        globalOrPerUser == bool.t, key,
        resolve
      ));
    }
    dataStoreGetKey({globalOrPerUser, pattern, index}) {
      ...
      GameJolt.DataStoreGetKeysEx(
        globalOrPerUser == bool.t, pattern,
        pResponse =>
      {
        if (pResponse.success == bool.f) { err.keys = pResponse.message; return; }
        if (!pResponse.keys) { data.keys = ""; err.keys = err.noIndex; return; }
