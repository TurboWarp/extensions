(function (Scratch) {
  ...
  renderer._shaderManager.reloadShader = function () {
    if (renderer._shaderManager._shaderCache.background[0]) {
      gl.deleteProgram(
        renderer._shaderManager._shaderCache.background[0].program,
      );
  ...
  class customshader {
    ...
    opCodeAddFragCode({ code }, { target }) {
      renderer.shader_frag_addCode = code;
    }
    opCodeAddVertCode({ code }, { target }) {
