...
(function (Scratch) {
  ...
  class InputBoxExtension {
    ...
    cmdSetMode(args) {
      try {
        const { ID, MODE } = args;
        if (MODE === "single" || MODE === "multiline")
          this._switchType(ID, MODE);
      } catch (e) {}
    }
    ...
    cmdSetEnterAction(args) {
      try {
        const { ID, MODE } = args;
        const meta = this.inputs.get(ID);
        if (meta && ["trigger", "newline", "both"].includes(MODE))
          meta.enterMode = MODE;
      } catch (e) {}
    }
      ...
      return {
        ...
        blocks: [
          //BT.COMMAND Blocks
          {
            ...
            text: tr(
              "create/move input box ID: [ID] x: [X] y: [Y] width: [WIDTH] height: [HEIGHT]",
          ...
          },
          {
            ...
            blockType: BT.COMMAND,
            text: tr(
              "when Enter pressed on input box [ID], do [MODE]in multiline mode"
            ),
            arguments: {
              ID: { type: "string", defaultValue: "inputbox" },
              MODE: {
          ...
          },
          {
            ...
            blockType: BT.BOOLEAN,
            text: tr(
              "when Enter pressed on input box [ID], is the mode [MODE]?"
            ),
            arguments: {
              ID: { type: "string", defaultValue: "inputbox" },
              MODE: {
