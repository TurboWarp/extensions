...
(function (Scratch) {
  ...
  const itemOfFromString = (no, from, splitby) =>
    Scratch.Cast.toString(from).split(splitby)[no] || "";
  ...
  async function getCoordinatesFromAdress(adress) {
    var response = await Scratch.fetch(
      `https://photon.komoot.io/api/?q=${encodeURIComponent(adress)}`
    )
      .then((r) => r.text())
      .catch(() => "");
    if (!response) return "";
        ...
        blocks: [
          {
            ...
            text: Scratch.translate(
              "is current [METEO] at latitude [latitude] longitude [longitude] [OPERATORS] [value] degrees"
            ),
            arguments: {
              METEO: {
          ...
          },
          {
            ...
            text: Scratch.translate(
              "current [METEO] at latitude [latitude] longitude [longitude]"
            ),
            arguments: {
              METEO: {
                type: Scratch.ArgumentType.STRING,
          ...
          "---",
          {
            ...
            blockType: Scratch.BlockType.BOOLEAN,
            text: Scratch.translate(
              "is current [METEO] in [place] [OPERATORS] [value] degrees"
            ),
            arguments: {
              METEO: {
                type: Scratch.ArgumentType.STRING,
        ...
        if (args.METEO.includes("(C)")) {
          ...
          return Scratch.Cast.toNumber(
            Scratch.Cast.toString(itemOfFromString(0, temperature, ".")) +
              "." +
              lettersOf(itemOfFromString(1, temperature, "."), 0, 1)
          );
        }
