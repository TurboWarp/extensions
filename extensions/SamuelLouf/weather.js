...
(function (Scratch) {
   ...
   * @returns {String}
   */
  const itemOfFromString = (no, from, splitby) => from.split(splitby)[no] || '';
    ...
    async get_current_weather_at(args) {
      ...
      if (args.METEO.includes('temperature')){
        if (args.METEO.includes('(C)')){
          ...
          return Number(String(itemOfFromString(0, temperature, '.')) + '.' + lettersOf(itemOfFromString(1, temperature, '.'), 0, 1));
        }
        ...
        if (args.METEO.includes('(km/h)')){
          ...
          return Number(String(itemOfFromString(0, wind_speed, '.')) + '.' + lettersOf(itemOfFromString(1, wind_speed, '.'), 0, 1));
        }
      } else {
