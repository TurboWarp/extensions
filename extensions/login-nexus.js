      ...
      try {
        ...
        if (response.ok) {
          ...
          this.setDebugMessage(
            "Registration successful: " + JSON.stringify(data)
          );
        } else {
    ...
    useService(args) {
      ...
      if (service === "Google") {
        if (action === "Register" || action === "Authenticate") {
          ...
          Scratch.openWindow(
            `https://accounts.google.com/o/oauth2/auth?client_id=${this.clientId}&redirect_uri=${this.redirectUri}&response_type=token&scope=email`,
            "_blank"
          );
        }
        ...
        if (action === "Register" || action === "Authenticate") {
          ...
          Scratch.openWindow(
            `https://login.microsoftonline.com/common/oauth2/v2.0/authorize?client_id=${this.clientId}&response_type=token&redirect_uri=${this.redirectUri}&scope=openid email`,
            "_blank"
          );
        }
      } else {
