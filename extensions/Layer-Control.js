...
(function (Scratch) {
  ...
  if (!Scratch.extensions.unsandboxed)
    throw new Error("Layer Control must run unsandboxed!");
  ...
  class SPlayering {
        ...
        blocks: [
          ...
          },
          {
            ...
            text: Scratch.translate(
              "swap layers of ID [TARGET] with ID [ORDER]"
            ),
            arguments: {
              ORDER: { type: Scratch.ArgumentType.NUMBER, defaultValue: 0 },
              TARGET: { type: Scratch.ArgumentType.NUMBER, defaultValue: 1 },
    ...
    reset() {
      ...
      const newDraw = [
        stage,
        ...allLay.filter((item) => ![stage, pen, video].includes(item)),
      ];
    ...
    moveID(args) {
      ...
      const targetL = Math.max(
        0,
        Math.min(allLay.length - 1, Math.round(args.ORDER))
      );
      if (index !== -1) {
    ...
    swapID(args) {
      ...
      if (
        target1 !== target2 &&
        target1 >= 0 &&
        target1 < allLay.length &&
        target2 >= 0 &&
        target2 < allLay.length
      ) {
