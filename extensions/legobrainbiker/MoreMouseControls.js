...
(function (Scratch) {
  ...
  const extId = "legobrainbikerMoreMouseControls";
  const extIcon =
  ...
  const deltaModes = ["px", "lines", "pages"];
  const scrollAxises = { X_AXIS: 0, Y_AXIS: 1, Z_AXIS: 2 };
  const scrollDirs = [
    { AXIS: "ax", NEG: "l", POS: "r" },
    { AXIS: "ay", NEG: "u", POS: "d" },
    { AXIS: "az", NEG: "o", POS: "i" },
  ...
  class MoreMouseControls {
    constructor() {
      ...
      this.scrollDisabled = runtime.extensionStorage[extId].scrollDisabled || false;
      runtime.ioDevices.mouse.usesRightClickDown = runtime.extensionStorage[extId].contextMenuDisabled || false;

      // An event listener is used instead of Mouse I/O API,
      ...
      canvas.addEventListener(
        ...
        (e) => {
          ...
          this.wheelDelta.mode = deltaModes[e.deltaMode] || "";

          runtime.startHats(`${extId}_onScroll`);
          this._startOnScrollHats(e.deltaX, scrollAxises.X_AXIS);
          this._startOnScrollHats(e.deltaY, scrollAxises.Y_AXIS);
          this._startOnScrollHats(e.deltaZ, scrollAxises.Z_AXIS);
        },
        { passive: true }
      );
      ...
      runtime.ioDevices.mouse.postData = function(data) {
        if (typeof data.isDown !== "undefined") {
          ...
          if (data.isDown) {
            runtime.startHats(`${extId}_onMouseDown`, {
              BUTTON: Cast.toString(button),
            });
          } else {
            runtime.startHats(`${extId}_onMouseUp`, {
              BUTTON: Cast.toString(button),
            });
          }
        }
        ...
        blocks: [
          ...
          "---",
          {
            ...
            arguments: {
              SCROLL_DIR: {
                menu: "SCROLL_DIRS",
                defaultValue: scrollDirs[scrollAxises.Y_AXIS].NEG,
              },
            },
          },
          {
            ...
            items: [
              {
                ...
                value: scrollDirs[scrollAxises.Y_AXIS].NEG,
              },
              {
                text: Scratch.translate("down"),
                value: scrollDirs[scrollAxises.Y_AXIS].POS,
              },
              {
                text: Scratch.translate("to the left"),
                value: scrollDirs[scrollAxises.X_AXIS].NEG,
              },
              {
                text: Scratch.translate("to the right"),
                value: scrollDirs[scrollAxises.X_AXIS].POS,
              },
              {
                text: Scratch.translate("out"),
                value: scrollDirs[scrollAxises.Z_AXIS].NEG,
              },
              {
                text: Scratch.translate("in"),
                value: scrollDirs[scrollAxises.Z_AXIS].POS,
              },
              {
                text: Scratch.translate("along the x-axis"),
                value: scrollDirs[scrollAxises.X_AXIS].AXIS,
              },
              {
                text: Scratch.translate("along the y-axis"),
                value: scrollDirs[scrollAxises.Y_AXIS].AXIS,
              },
              {
                text: Scratch.translate("along the z-axis"),
                value: scrollDirs[scrollAxises.Z_AXIS].AXIS,
              },
            ],
          },
          DELTA_TYPE: {
    ...
    _startOnScrollHats(delta, axis) {
      if (delta === 0) return;
      runtime.startHats(`${extId}_onScrollInDir`, {
        SCROLL_DIR: scrollDirs[axis].AXIS,
      });
      runtime.startHats(`${extId}_onScrollInDir`, {
        SCROLL_DIR: delta > 0
          ? scrollDirs[axis].POS
    ...
    isScrolledInDir(args) {
      ...
      const wheelDeltaArray = [this.wheelDelta.x, this.wheelDelta.y, this.wheelDelta.z];
      scrollDirs.forEach((axisEnums, i) => {
    ...
    toggleContextMenu(args) {
      ...
      runtime.extensionStorage[extId].contextMenuDisabled = runtime.ioDevices.mouse.usesRightClickDown;
    }
    ...
    toggleScroll(args) {
      ...
      runtime.extensionStorage[extId].scrollDisabled = this.scrollDisabled;
    }
  }
