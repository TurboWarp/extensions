	...
	class LiFS {
		...
		sync({
			...
			if (!this.hasPermission(path1, "delete")) {
				return this._setError(`Rename failed: No 'delete' permission on ${path1}`);
			}
			if (this.fs.has(path2)) {
				return this._setError(`Rename failed: Destination ${path2} already exists`);
			}
			if (!this.hasPermission(path2, "create")) {
				return this._setError(`Rename failed: No 'create' permission for ${path2}`);
			}
		...
		copy({
			...
			if (this.fs.has(path2)) {
				return this._setError(`Copy failed: Destination ${path2} already exists`);
			}
			if (!this.hasPermission(path2, "create")) {
				return this._setError(`Copy failed: No 'create' permission for ${path2}`);
			}
		...
		start({
			...
			if (this._isPathDir(path) && path.length > 1) {
				const fileName = path.substring(0, path.length - 1).split("/").pop();
			...
			if (path === "/") {
				return this._setError("Create failed: Cannot create root directory '/'");
			}
		...
		del({
			...
			if (!this.hasPermission(path, "delete")) {
				return this._setError(`Delete failed: No 'delete' permission on ${path}`);
			}
		...
		in({
			...
			try {
				...
				if (version === "1.0.5") {
					...
					if (!data.fs || typeof data.fs !== 'object' || Array.isArray(data.fs)) {
						return this._setError("Import failed: v1.0.5 data is corrupt (missing 'fs' object).");
					}
					migrationData = data.fs;

				} else if (version === "1.0.4" || version === "1.0.3" || version === "1.0.2") {
					...
					if (!Array.isArray(data.fi) || !Array.isArray(data.sy) ||
					...
					return this._setError(`Import failed: Incompatible version "${version}". Expected "${extensionVersion}" or older.`);
				}
				...
				if (needsMigration) {
					...
					for (let i = 0; i < data.sy.length; i++) {
						...
						if (
							...
							typeof perm !== "object" || perm === null || Array.isArray(perm) ||
							typeof limit !== "number" ||
							typeof perm.create !== "boolean" || typeof perm.delete !== "boolean" ||
							typeof perm.see !== "boolean" || typeof perm.read !== "boolean" ||
							typeof perm.write !== "boolean" || typeof perm.control !== "boolean"
						) {
							this._setError("Import failed: Corrupt data found in legacy filesystem entries.");
							this._internalClean();
							return;
						}
					...
					for (const path in data.fs) {
						if (Object.prototype.hasOwnProperty.call(data.fs, path)) {
							...
							if (!entry || typeof entry.perms !== 'object' || typeof entry.limit !== 'number' ||
								typeof entry.created !== 'number' || typeof entry.modified !== 'number' ||
								typeof entry.accessed !== 'number') {
								this._setError(`Import failed: Corrupt entry for path "${path}".`);
								this._internalClean();
								return;
							}
					...
					if (!this.fs.has("/")) {
						this._setError("Import failed: Rebuilt filesystem is missing root '/'.");
						this._internalClean();
						return;
					}
				}
		...
		setPerm({
			...
			if (!this.hasPermission(path, "control")) {
				return this._setError(`setPerm failed: No 'control' permission on ${path}`);
			}
			...
			for (const [currentPath, entry] of this.fs.entries()) {
				if (
					(isDir && currentPath.startsWith(path)) ||
					currentPath === path
				) {
		...
		setLimit({
			...
			if (!this._isPathDir(path)) {
				return this._setError("setLimit failed: Path must be a directory (end with /)");
			}
			if (!this.hasPermission(path, "control")) {
				return this._setError(`setLimit failed: No 'control' permission on ${path}`);
			}
			const entry = this.fs.get(path);
			if (!entry) {
		...
		removeLimit({
			...
			if (!this._isPathDir(path)) {
				return this._setError("removeLimit failed: Path must be a directory (end with /)");
			}
			if (!this.hasPermission(path, "control")) {
				return this._setError(`removeLimit failed: No 'control' permission on ${path}`);
			}
			const entry = this.fs.get(path);
			if (!entry) {
				return this._setError(`removeLimit failed: Directory ${path} not found`);
			}
