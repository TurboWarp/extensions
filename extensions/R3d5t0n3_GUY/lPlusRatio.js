        ...
        blocks: [
          {
            ...
            text: Scratch.translate("const [mathConst]"),
            disableMonitor: true,
            arguments: {
              mathConst: {
      ...
      return ((m) => {
        switch (m) {
          ...
          case "Euler's number":
            return Math.exp(1);
          case "golden ratio":
            return Math.sqrt(1.25) + 0.5;
          case "golden angle":
            return 180 * (3 - Math.sqrt(5));
          case "Lévy's constant":
            return Math.acos(-1) ** 2 / (12 * Math.log(2));
          case "Erdõ-Tenenbaum-Ford constant":
            return 1 - (1 + Math.log(Math.log(2))) / Math.log(2);
          case "Connective Constant for hexagonal lattice":
            return Math.sqrt(2 + Math.sqrt(2));
          case "plastic ratio":
            return (
              Math.cbrt(0.5 + Math.sqrt(69) / 18) +
              Math.cbrt(0.5 - Math.sqrt(69) / 18)
            );
          case "silver ratio":
            return Math.sqrt(2) + 1;
          case "tau":
            return 2 * Math.acos(-1);
          case "supergolden ratio":
            return (
              (1 +
                Math.cbrt((29 + 3 * Math.sqrt(93)) / 2) +
                Math.cbrt((29 - 3 * Math.sqrt(93)) / 2)) /
              3
            );
          default:
