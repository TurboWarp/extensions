...
(function (Scratch) {
  ...
  const projectId = (location.href.match(
    "project_url=https?://api.hatch.lol/projects/([0-9]+)/content"
  ) || [-1, -1])[1];
  ...
  class HatchDotLol {
    ...
    async status(args) {
      try {
        if (args.SERVER === "main")
          ...
          return (await fetch("https://api.hatch.lol/")).status;
        if (args.SERVER === "save state")
          // eslint-disable-next-line extension/use-scratch-fetch
          return (await fetch("https://hatchsaves.raynec.dev/")).status;
      } catch {
        return 500;
      }
    }
    ...
    async ok(args) {
      try {
        if (args.SERVER === "main")
          // eslint-disable-next-line extension/use-scratch-fetch
          return (await fetch("https://api.hatch.lol/")).ok;
        if (args.SERVER === "save state")
          // eslint-disable-next-line extension/use-scratch-fetch
          return (await fetch("https://hatchsaves.raynec.dev/")).ok;
      } catch {
        return false;
      }
    }
    ...
    async save_data(args) {
      ...
      try {
        ...
        if (res.status === 200) {
          ...
          // eslint-disable-next-line extension/use-scratch-fetch
          res = await fetch(
            `https://hatchsaves.raynec.dev/${projectId}/${user.id}`
          );
          if (res.ok) {
            let data = await res.json();
            return data.data;
