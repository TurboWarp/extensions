  ...
  class WheelScroll {
    constructor() {
      ...
      const startWhenScrollEvent = (
        ...
        threads.forEach((thread) => {
          thread.Den4ik12WheelScroll = {
            deltaX,
            deltaY,
            deltaZ,
            deltaMode: deltaModes[deltaMode],
          };
        });
      };
      ...
      canvas.addEventListener(
        ...
        (e) => {
          ...
          deltaY += e.deltaY;
          deltaZ += e.deltaZ;
          deltaMode = deltaModes[e.deltaMode];
    ...
    getInfo() {
      return {
        ...
        blocks: [
          ...
          "---",
          {
            opcode: "isScrolledDir",
            text: Scratch.translate("mouse wheel scrolled [SCROLL_TYPE]?"),
    ...
    getDelta(args, util) {
      ...
      switch (args.DELTA_TYPE) {
        case "x": {
          result = util.thread?.Den4ik12WheelScroll?.deltaX || deltaX;
          break;
        }
        case "y": {
          result = util.thread?.Den4ik12WheelScroll?.deltaY || deltaY;
          break;
        }
        case "z": {
          result = util.thread?.Den4ik12WheelScroll?.deltaZ || deltaZ;
          break;
        }
        case "mode": {
          result = util.thread?.Den4ik12WheelScroll?.deltaMode || deltaMode;
          break;
        }
        default:
          return "";
