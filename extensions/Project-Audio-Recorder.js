  ...
  class ProjectAudioRecorder {
    getInfo() {
      return {
        ...
        name: 'Project Audio Recorder',
        color1: '#202d40',
        blocks: [
          {
            opcode: 'startRecording',
            blockType: Scratch.BlockType.COMMAND,
            text: 'Start recording project audio'
          },
          {
            opcode: 'stopAndDownloadMp3',
            blockType: Scratch.BlockType.COMMAND,
            text: 'Stop recording and download .mp3 file name [NAME]',
            arguments: {
              NAME: {
          ...
          },
          {
            ...
            blockType: Scratch.BlockType.COMMAND,
            text: 'Stop recording and download .wav file name [NAME]',
            arguments: {
              NAME: {
                type: Scratch.ArgumentType.STRING,
                defaultValue: 'My Audio'
    ...
    startRecording() {
      ...
      if (!vm || !vm.runtime || !vm.runtime.audioEngine) {
        alert('TurboWarp Ses Motoru bulunamadı!');
        return;
      }
      ...
      sourceNode = audioEngine.inputNode || audioEngine.masterGain || audioEngine._masterGain;

      if (!sourceNode) {
        alert('Ses motoru düğümü bulunamadı! Projede önce yeşil bayrağa basıp kısa bir ses tetikleyin.');
        return;
      }
    ...
    _stopAndDownload(fileName, ext) {
      ...
      mediaRecorder.onstop = () => {
        ...
        if (sourceNode && recorderDest) {
          try { sourceNode.disconnect(recorderDest); } catch (e) {}
        }
