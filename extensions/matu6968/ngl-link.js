...
(function(Scratch) {
  ...
  class NGLLink {
    ...
    async loadFakeQuestions(langCode) {
      ...
      try {
        const response = await Scratch.fetch(`https://cdn.simplelocalize.io/57157aec81d54cb6b2a43f8b34a61d47/_production/${normalizedLang}`);
        ...
        console.log(`Loaded ${this.fakeQuestions.length} fake questions for language: ${normalizedLang}`);
      } catch (err) {
        console.error(`Failed to load fake questions for language ${normalizedLang}:`, err);
      }
    }
    ...
    getRandomQuestion() {
      ...
      return this.fakeQuestions[Math.floor(Math.random() * this.fakeQuestions.length)];
    }
