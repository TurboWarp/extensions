...
(function(Scratch) {
  ...
  const isNode = req !== null && proc !== null;
  const fs = isNode ? req('fs') : null;
  const path = isNode ? req('path') : null;
  const crypto = isNode ? req('crypto') : null;
  const os = isNode ? req('os') : null;

  class TurboGuard {
    getInfo() {
      return {
        id: 'turboguard',
        name: Scratch.translate({ id: 'turboguard.name', default: 'TurboGuard' }),
        color1: '#f26a1b',
        color2: '#d95316',
        color3: '#bf400b',
        ...
        blocks: [
          {
            ...
            text: Scratch.translate({ id: 'turboguard.getHash', default: 'file hash [FILE]' }),
            arguments: {
              FILE: {
          ...
          },
          {
            opcode: 'getFileSize',
            blockType: Scratch.BlockType.REPORTER,
            text: Scratch.translate({ id: 'turboguard.getFileSize', default: 'file size in bytes [FILE]' }),
            arguments: {
              FILE: {
                type: Scratch.ArgumentType.STRING,
                defaultValue: 'package.json'
          ...
          },
          {
            ...
            text: Scratch.translate({ id: 'turboguard.fileExists', default: 'file [FILE] exists?' }),
            arguments: {
              FILE: {
                type: Scratch.ArgumentType.STRING,
                defaultValue: 'package.json'
          ...
          },
          {
            opcode: 'readFile',
            blockType: Scratch.BlockType.REPORTER,
            text: Scratch.translate({ id: 'turboguard.readFile', default: 'contents of file [FILE]' }),
            arguments: {
              FILE: {
                type: Scratch.ArgumentType.STRING,
                defaultValue: 'package.json'
          ...
          },
          {
            ...
            text: Scratch.translate({ id: 'turboguard.writeFile', default: 'rewrite file [FILE] with [TEXT]' }),
            arguments: {
              FILE: {
                type: Scratch.ArgumentType.STRING,
          ...
          },
          {
            opcode: 'getHWID',
            blockType: Scratch.BlockType.REPORTER,
            text: Scratch.translate({ id: 'turboguard.getHWID', default: 'system HWID' })
          },
          {
            opcode: 'getProcessPath',
            blockType: Scratch.BlockType.REPORTER,
            text: Scratch.translate({ id: 'turboguard.getProcessPath', default: 'game directory path' })
          },
          {
            opcode: 'getFullReport',
            blockType: Scratch.BlockType.REPORTER,
            text: Scratch.translate({ id: 'turboguard.getFullReport', default: 'full system report' })
          }
        ]
      };
    }
    ...
    getHash(args) {
      ...
      try {
        const safePath = path.join(proc.cwd(), ...args.FILE.split(/[/\\]/));
    ...
    getHWID() {
      if (!isNode) return 'NO_NODE';
      try {
        const rawHwid = os.hostname() + '_' + os.userInfo().username + '_' + os.arch();
