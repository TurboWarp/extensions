(Scratch => {
  ...
  const number2binary = number => {
    return Scratch.Cast.toNumber(number).toString(2);
  };
  const binary2number = binary => {
    return /^-?[01]+$/.test(binary) ? (parseInt(binary, 2) || 0) : 0;
  };

  const isNumberBits = bits => {
    return /^[0-1]{32}$/.test(bits);
  };
  ...
  class Bitwise {
    ...
    isTrueNumberBits({CENTRAL}) {
      return isNumberBits(CENTRAL);
    }
    toTrueNumberBits({CENTRAL}) {
      return number2bits(CENTRAL);
    }
    ofTrueNumberBits({CENTRAL}) {
    ...
    isNumberBits({CENTRAL}) {
      return isBinary(CENTRAL);
    }
    toNumberBits({CENTRAL}) {
      return number2binary(CENTRAL);
    }
    ofNumberBits({CENTRAL}) {
      return binary2number(CENTRAL);
    }
    // UNUSED
  }
