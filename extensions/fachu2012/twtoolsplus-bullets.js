// Name: Tools+ Bullets
// ID: twtoolsplusbullets
// Description: Bullet and projectile utilities for TurboWarp.
// By: fachu2012
// License: MPL-2.0
...
//
// twtoolsplus-bullets.js — only Tools+ Bullets.
// Standalone pack: only Tools+ Bullets (twtoolsplusbullets).
//
...
(function (Scratch) {
  ...
  class TWToolsPlusCore {
    ...
    mathMap(args) {
      const t =
        (toNumber(args.VALUE) - toNumber(args.A)) /
        (toNumber(args.B) - toNumber(args.A) || 1);
      return toNumber(args.C) + t * (toNumber(args.D) - toNumber(args.C));
    }
    mathClamp(args) {
      return Math.min(
        Math.max(toNumber(args.VALUE), toNumber(args.MIN)),
        toNumber(args.MAX)
      );
    }
    mathDistance(args) {
    ...
    mathSmoothDamp(args) {
      const c = toNumber(args.CURRENT),
        t = toNumber(args.TARGET),
        s = Math.max(0, Math.min(1, toNumber(args.SPEED)));
      return c + (t - c) * s;
    }
    mathNoise1D(args) {
    ...
    textReplace(args) {
      return String(args.TEXT)
        .split(String(args.FIND))
        .join(String(args.REPLACE));
    }
    textContains(args) {
    ...
    textTruncate(args) {
      const t = String(args.TEXT),
        n = Math.max(0, Math.round(toNumber(args.N)));
    ...
    textCharsToList(args) {
      return JSON.stringify([...String(args.TEXT)]);
    }
    textBase64(args) {
      try {
        if (args.MODE === "encode")
          return btoa(unescape(encodeURIComponent(String(args.TEXT))));
    ...
    // ===== LISTS =====
    listReverse(args) {
      return this._withArray(args.LIST, (arr) => [...arr].reverse());
    }
    listShuffle(args) {
    ...
    listRemoveDuplicates(args) {
      return this._withArray(args.LIST, (arr) => [...new Set(arr)]);
    }
    listSort(args) {
      return this._withArray(args.LIST, (arr) => {
        const copy = [...arr];
        if (args.MODE === "numeric")
          copy.sort((a, b) => toNumber(a) - toNumber(b));
    ...
    listSublist(args) {
      return this._withArray(args.LIST, (arr) =>
        arr.slice(toNumber(args.I) - 1, toNumber(args.J))
      );
    }
    listMerge(args) {
      return JSON.stringify([
        ...this._parseArray(args.LIST1),
        ...this._parseArray(args.LIST2),
      ]);
    }
    listEquals(args) {
      return (
        JSON.stringify(this._parseArray(args.LIST1)) ===
        JSON.stringify(this._parseArray(args.LIST2))
      );
    }
    ...
    listWeightedPick(args) {
      ...
      const weights = this._parseArray(args.WEIGHTS).map((w) =>
        Math.max(0, toNumber(w))
      );
    ...
    // ===== JSON =====
    jsonListToJson(args) {
      return JSON.stringify(this._parseArray(args.LIST));
    }
    jsonJsonToList(args) {
      return JSON.stringify(this._parseArray(args.JSON));
    }
    jsonGetPath(args) {
      try {
    ...
    storageSave(args) {
      try {
        localStorage.setItem(storageKey(args.KEY), args.VALUE);
      } catch (e) {}
    ...
    storageDelete(args) {
      try {
        localStorage.removeItem(storageKey(args.KEY));
      } catch (e) {}
    ...
    spriteTouchingEdge(args, util) {
      ...
      const halfW = this.runtime.stageWidth / 2,
        halfH = this.runtime.stageHeight / 2;
      return (
        bounds.left <= -halfW ||
        bounds.right >= halfW ||
        bounds.top >= halfH ||
    ...
    spriteCostumeSize(args, util) {
      ...
      const dim =
        args.DIMENSION === "width" ? costume.size[0] : costume.size[1];
    ...
    spriteApplyFriction(args) {
      return toNumber(args.V) * toNumber(args.F);
    }
    ...
    spriteSmoothFollow(args, util) {
      ...
      if (args.AXIS === "x")
        return util.target.x + (toNumber(args.TX) - util.target.x) * f;
    ...
    spriteMouseAngle(args, util) {
      ...
      try {
        ...
        const my = this.runtime.ioDevices.mouse.getScratchY();
        return (
          (Math.atan2(mx - util.target.x, my - util.target.y) * 180) / Math.PI
        );
      } catch (e) {
        return 0;
      }
    ...
    spriteKnockback(args, util) {
      ...
      util.target.setXY(
        util.target.x + (dx / len) * a,
        util.target.y + (dy / len) * a
      );
    }
    ...
    timeDaysBetween(args) {
      return Math.round(
        Math.abs(new Date(args.DATE2) - new Date(args.DATE1)) / 86400000
      );
    }
    ...
    timeStopwatch(args) {
      ...
      if (!sw) {
        sw = { start: 0, pausedAt: 0, accumulated: 0, running: false };
        this._stopwatches.set(id, sw);
      }
      const now = Date.now();
      ...
      if (args.OP === "pause") {
        if (sw.running) {
          sw.accumulated += (now - sw.start) / 1000;
          sw.running = false;
          sw.pausedAt = now;
        }
    ...
    // ===== DEBUG =====
    debugLog(args) {
      console.log("[TW Tools+]", args.VALUE);
    }
    debugLogLevel(args) {
      const level =
        args.LEVEL === "error"
    ...
    debugAssert(args) {
      if (!args.CONDITION)
        console.error("[TW Tools+] Assertion failed:", args.TEXT);
    }

    // ===== COLOR =====
    colorRgbToHex(args) {
      const clamp255 = (n) =>
        Math.max(0, Math.min(255, Math.round(toNumber(n))));
    ...
    dictGet(args) {
      try {
        const obj = JSON.parse(args.DICT || "{}");
        return Object.prototype.hasOwnProperty.call(obj, args.KEY)
          ? obj[args.KEY]
          : args.DEFAULT;
      } catch (e) {
        return args.DEFAULT;
    ...
    dictKeysOrValues(args) {
      try {
        const obj = JSON.parse(args.DICT || "{}");
        return JSON.stringify(
          args.MODE === "keys" ? Object.keys(obj) : Object.values(obj)
        );
      } catch (e) {
        return "[]";
      }
    ...
    netGetQueryParam(args) {
      try {
        return new URL(args.URL).searchParams.get(args.KEY) || "";
      } catch (e) {
        return "";
      }
    }
    ...
    _findSoundPlayer(soundName, util) {
      try {
        const sound = util.target.sprite.sounds.find(
          (s) => s.name === soundName
        );
          ...
          try {
            ...
            if (broadcastVar)
              this.runtime.startHats("event_whenbroadcastreceived", {
                BROADCAST_OPTION: broadcastVar.id,
              });
          } catch (e) {}
        },
    ...
    controlTaskExists(args) {
      return this._tasks.has(args.ID);
    }
    ...
    geoPointInRect(args) {
      ...
      return (
        toNumber(args.X) >= minX &&
        toNumber(args.X) <= maxX &&
        toNumber(args.Y) >= minY &&
        toNumber(args.Y) <= maxY
      );
    }
    ...
    bulletVelocityX(args, util) {
      const tx = toNumber(args.X),
        ty = toNumber(args.Y);
      const x = util.target ? util.target.x : 0,
        y = util.target ? util.target.y : 0;
      const dx = tx - x,
        dy = ty - y;
    ...
    bulletPointInDirectionOfMotion(args, util) {
      ...
      util.target.setDirection(
        (Math.atan2(toNumber(args.VX), toNumber(args.VY)) * 180) / Math.PI
      );
    }
    ...
    bulletIsOffStage(args, util) {
      ...
      const halfW = this.runtime.stageWidth / 2 + m,
        halfH = this.runtime.stageHeight / 2 + m;
      const x = util.target.x,
        y = util.target.y;
    ...
    bulletHomingDir(args, util) {
      ...
      const target = util.target
        ? (Math.atan2(
            toNumber(args.X) - util.target.x,
            toNumber(args.Y) - util.target.y
          ) *
            180) /
          Math.PI
    ...
    bulletAccelerate(args) {
      return Math.min(
        toNumber(args.MAX),
        toNumber(args.SPEED) + toNumber(args.ACCEL)
      );
    }
    ...
    bulletSetLifetime(args, util) {
      if (!util.target) return;
      this._bulletLifetimes.set(
        util.target.id,
        Math.max(0, Math.round(toNumber(args.FRAMES)))
      );
    }
    ...
    miscDeepCopy(args) {
      try {
        return JSON.stringify(
          JSON.parse(JSON.stringify(JSON.parse(args.JSON)))
        );
      } catch (e) {
        return args.JSON;
      }
  ...
  const SHARED_MENUS = {
    parityMenu: {
      items: [Scratch.translate("even"), Scratch.translate("odd")],
    },
    gcdLcmMenu: { items: [Scratch.translate("GCD"), Scratch.translate("LCM")] },
    numberFormatMenu: {
      items: [Scratch.translate("Roman numeral"), Scratch.translate("words")],
    },
    caseMenu: {
      items: [
        Scratch.translate("UPPERCASE"),
        Scratch.translate("lowercase"),
        Scratch.translate("Capitalized"),
      ],
    },
    padSideMenu: {
      items: [Scratch.translate("start"), Scratch.translate("end")],
    },
    sortModeMenu: {
      items: [Scratch.translate("numeric"), Scratch.translate("alphabetical")],
    },
    minMaxMenu: { items: [Scratch.translate("max"), Scratch.translate("min")] },
    sumAverageMenu: {
      items: [Scratch.translate("sum"), Scratch.translate("average")],
    },
    axisMenu: { items: [Scratch.translate("x"), Scratch.translate("y")] },
    dimensionMenu: {
      items: [Scratch.translate("width"), Scratch.translate("height")],
    },
    boxSideMenu: {
      items: [
        Scratch.translate("top"),
        Scratch.translate("bottom"),
        Scratch.translate("left"),
        Scratch.translate("right"),
      ],
    },
    mouseButtonMenu: {
      items: [
        Scratch.translate("left"),
        Scratch.translate("right"),
        Scratch.translate("middle"),
      ],
    },
    logLevelMenu: {
      items: [
        Scratch.translate("info"),
        Scratch.translate("warning"),
    ...
    keysValuesMenu: {
      items: [Scratch.translate("keys"), Scratch.translate("values")],
    },
    urlEncodeMenu: {
      items: [Scratch.translate("URL-encode"), Scratch.translate("URL-decode")],
    },
    escapeMenu: {
      items: [Scratch.translate("escape"), Scratch.translate("unescape")],
    },
    base64Menu: {
      items: [Scratch.translate("encode"), Scratch.translate("decode")],
    },
    compareOpMenu: { items: [">", ">=", "<", "<=", "=="] },
    mapOpMenu: { items: ["+", "-", "*", "/"] },
    stackOpMenu: {
      items: [
        Scratch.translate("push"),
        Scratch.translate("pop"),
        Scratch.translate("peek"),
      ],
    },
    stopwatchMenu: {
      items: [
        Scratch.translate("start"),
        Scratch.translate("pause"),
        Scratch.translate("resume"),
        Scratch.translate("read"),
        Scratch.translate("reset"),
      ],
    },
    easingMenu: {
      items: [
        Scratch.translate("linear"),
        Scratch.translate("ease-in"),
        Scratch.translate("ease-out"),
        Scratch.translate("ease-in-out"),
      ],
    },
    invertGrayMenu: {
      items: [Scratch.translate("invert"), Scratch.translate("grayscale")],
    },
  };
    ...
    [
      label("Bullets"),
      {
        ...
        text: Scratch.translate(
          "velocity X towards ([X],[Y]) at speed [SPEED]"
        ),
        arguments: {
      ...
      },
      {
        ...
        blockType: Scratch.BlockType.REPORTER,
        text: Scratch.translate(
          "velocity Y towards ([X],[Y]) at speed [SPEED]"
        ),
        arguments: {
          X: { type: Scratch.ArgumentType.NUMBER, defaultValue: 0 },
          Y: { type: Scratch.ArgumentType.NUMBER, defaultValue: 0 },
      ...
      },
      {
        ...
        blockType: Scratch.BlockType.REPORTER,
        text: Scratch.translate("direction towards ([X],[Y])"),
        arguments: {
          X: { type: Scratch.ArgumentType.NUMBER, defaultValue: 0 },
          Y: { type: Scratch.ArgumentType.NUMBER, defaultValue: 0 },
        },
      },
      {
        ...
        blockType: Scratch.BlockType.REPORTER,
        text: Scratch.translate(
          "velocity [AXIS] from direction [DIR] speed [SPEED]"
        ),
        arguments: {
          AXIS: {
      ...
      },
      {
        ...
        blockType: Scratch.BlockType.REPORTER,
        text: Scratch.translate("speed from velocity ([VX],[VY])"),
        arguments: {
      ...
      },
      {
        ...
        blockType: Scratch.BlockType.REPORTER,
        text: Scratch.translate("angle difference from [DIR1] to [DIR2]"),
        arguments: {
      ...
      },
      {
        ...
        blockType: Scratch.BlockType.REPORTER,
        text: Scratch.translate(
          "spread direction [DIR] by [SPREAD] (index [I] of [N])"
        ),
        arguments: {
          DIR: { type: Scratch.ArgumentType.NUMBER, defaultValue: 90 },
      ...
      },
      {
        ...
        text: Scratch.translate("point in direction of velocity ([VX],[VY])"),
        arguments: {
          VX: { type: Scratch.ArgumentType.NUMBER, defaultValue: 3 },
          VY: { type: Scratch.ArgumentType.NUMBER, defaultValue: 4 },
        },
      },
      {
        ...
        blockType: Scratch.BlockType.COMMAND,
        text: Scratch.translate("move by velocity ([VX],[VY])"),
        arguments: {
          VX: { type: Scratch.ArgumentType.NUMBER, defaultValue: 3 },
          VY: { type: Scratch.ArgumentType.NUMBER, defaultValue: 4 },
        },
      },
      {
        ...
        text: Scratch.translate("is this sprite off stage (margin [MARGIN])?"),
        arguments: {
          MARGIN: { type: Scratch.ArgumentType.NUMBER, defaultValue: 20 },
        },
      },
      {
        ...
        blockType: Scratch.BlockType.REPORTER,
        text: Scratch.translate(
          "homing: turn [CURRENT] toward ([X],[Y]) max [MAX] °/frame"
        ),
        arguments: {
      ...
      },
      {
        ...
        blockType: Scratch.BlockType.REPORTER,
        text: Scratch.translate(
          "accelerate speed [SPEED] by [ACCEL] (max [MAX])"
        ),
        arguments: {
      ...
      },
      {
        ...
        blockType: Scratch.BlockType.REPORTER,
        text: Scratch.translate(
          "circle pattern direction index [I] of [N] (offset [OFFSET])"
        ),
        arguments: {
          I: { type: Scratch.ArgumentType.NUMBER, defaultValue: 1 },
      ...
      },
      {
        ...
        blockType: Scratch.BlockType.COMMAND,
        text: Scratch.translate("set bullet lifetime to [FRAMES] frames"),
        arguments: {
          FRAMES: { type: Scratch.ArgumentType.NUMBER, defaultValue: 120 },
        },
      },
      {
        opcode: "bulletTickLifetime",
        blockType: Scratch.BlockType.BOOLEAN,
        text: Scratch.translate("bullet lifetime expired?"),
      },
    ]
  );
})(Scratch);
