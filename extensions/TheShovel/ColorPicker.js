(function(Scratch) {
    ...
    class ColorPicker {
        ...
        showPicker() {
            // @ts-ignore
            colorPicker.style.marginLeft = window.innerWidth / 2 + xpos + 'px';
            // @ts-ignore
            colorPicker.style.marginTop = window.innerHeight / 2 - ypos + 'px';
            setTimeout(function() {
                // @ts-ignore
                colorPicker.click();
            }, 5);
        ...
        setPos(args) {
            if (Math.abs(args.X) > window.innerWidth / 2 - 50) {
                if (args.X > 0) {
                    xpos = window.innerWidth / 2 - 50;
                } else {
                    xpos = 0 - window.innerWidth / 2 + 50;
                }
            } else {
                xpos = args.X;
            }
            if (Math.abs(args.Y) > window.innerHeight / 2 - 30) {
                if (args.Y > 0) {
                    ypos = window.innerHeight / 2 - 30;
                } else {
                    ypos = 0 - window.innerHeight / 2 + 30;
                }
            } else {
                ypos = args.Y;
            }
