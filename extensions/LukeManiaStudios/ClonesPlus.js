(function(Scratch) {
  ...
  class ClonesPlus {
      ...
      return {
        ...
        blocks: [
          ...
          },
          {
            ...
            blockType: Scratch.BlockType.COMMAND,
            text: 'stop scripts in clone with [INPUTA] set to [INPUTB]',
            filter: [Scratch.TargetType.SPRITE],
            arguments: {
              INPUTA: {
                type: Scratch.ArgumentType.STRING,
          ...
          },
          {
            ...
            blockType: Scratch.BlockType.COMMAND,
            text: 'delete clone with [INPUTA] set to [INPUTB]',
            filter: [Scratch.TargetType.SPRITE],
            arguments: {
              INPUTA: {
                type: Scratch.ArgumentType.STRING,
    ...
    setVariableOfClone(args, util) {
      if (!args.INPUTA) { return ''; }
      if (!args.INPUTC) { return ''; }
      const variableToEdit = JSON.parse(this.getVariableObj(args.INPUTA, util.target));
      const newVariableValue = args.INPUTB;
      const cloneVariableObj = JSON.parse(this.getVariableObj(args.INPUTC, util.target));
    ...
    getVariableOfClone(args, util) {
      if (!args.INPUTB) { return ''; }
      const clone = this.getCloneFromVariable(args.INPUTB, args.INPUTC, util.target.sprite.clones, 'obj');
      if (!clone) { return ''; }
      try {
        const variableObj = JSON.parse(this.getVariableObj(args.INPUTA, util.target));
        return clone.lookupOrCreateVariable(variableObj.id, variableObj.name).value;
      } catch (error){
        return '';
      }
    }
    ...
    getVariableOfMainSprite(args, util) {
      ...
      const variableObj = JSON.parse(this.getVariableObj(args.INPUT, util.target));
    ...
    getThingOfClone(args, util) {
      const clone = this.getCloneFromVariable(args.INPUTB, args.INPUTC, util.target.sprite.clones, 'obj');
      if (!clone) { return ''; }
      if (args.INPUTA === 'x position') { return clone.x; }
      if (args.INPUTA === 'y position') { return clone.y; }
      if (args.INPUTA === 'direction') { return clone.direction; }
      if (args.INPUTA === 'costume num') { return (clone.currentCostume + 1); }
      if (args.INPUTA === 'costume name') { return clone.getCostumes()[clone.currentCostume].name; }
      if (args.INPUTA === 'size') { return clone.size; }
      if (args.INPUTA === 'volume') { return clone.volume; }
    }

    getThingOfMainSprite(args, util) {
      const main = util.target.sprite.clones[0];
      if (args.INPUT === 'x position') { return main.x; }
      if (args.INPUT === 'y position') { return main.y; }
      if (args.INPUT === 'direction') { return main.direction; }
      if (args.INPUT === 'costume num') { return (main.currentCostume + 1); }
      if (args.INPUT === 'costume name') { return main.getCostumes()[main.currentCostume].name; }
      if (args.INPUT === 'size') { return main.size; }
      if (args.INPUT === 'volume') { return main.volume; }
    }
    ...
    getCloneFromVariable(variableObject, cloneVarValue, targetClones, returnOption) {
      try {
        const variableObj = JSON.parse(this.getVariableObj(variableObject, Scratch.vm.runtime._editingTarget));
        var clone = '';
    ...
    getVariables() {
      let varNames = [];
      const localVars = Scratch.vm.runtime._editingTarget.variables
    ...
    getVariableObj(name, target) {
      ...
      return JSON.stringify(variableObj);
    }
  }
