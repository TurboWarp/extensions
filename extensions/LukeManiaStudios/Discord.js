/*!
 * Discord API - By LukeManiaStudios - Version 1.0
 */
...
(function (Scratch) {
  ...
  async function createNewWebhook() {
    let name = prompt('Enter Webhook Name');
    if (!name) {
      return;
    }
    let url = prompt('Enter Webhook URL');
    if (!url) {
      return;
    }
  ...
  function createNewEmbed() {
    let name = prompt('Enter Embed Name');
    if (!name) {
      return;
    }
    embeds[name] = {
  ...
  let experimentsDisabled = true;
  let webhooks = Object.create(null);
  let embeds = Object.create(null);
  ...
  class DiscordAPI {
    getInfo() {
      return {
        ...
        blocks: [
          {
            opcode: 'sendBasicMessage',
            blockType: Scratch.BlockType.COMMAND,
            text: 'send [TYPE] [STRING] to url [WEBHOOK]',
            arguments: {
              TYPE: {
          ...
          },
          {
            ...
            hideFromPalette: !experimentsDisabled
          },

          '---',

          label('Webhooks', experimentsDisabled),
          ...
          label('Embeds', experimentsDisabled),
    ...
    deleteWebhook() {
      ...
      Reflect.deleteProperty(webhooks, name);
      Scratch.vm.extensionManager.refreshBlocks();
    ...
    deleteEmbed() {
      ...
      Reflect.deleteProperty(embeds, name);
      Scratch.vm.extensionManager.refreshBlocks();
    ...
    createWebhookMenu() {
      ...
      for (let index = 0; index < length; index++) {
        let name = Object.keys(webhooks)[index];
        webhookMenu.push({
          text: name,
          value: name
        });
      }
      const newWebhook = {text: 'New webhook', value: () => createNewWebhook()};
    ...
    createEmbedMenu() {
      ...
      for (let index = 0; index < length; index++) {
        let name = Object.keys(embeds)[index];
        embedMenu.push({
          text: name,
          value: name
        });
      }
      const newEmbed = {text: 'New embed', value: () => createNewEmbed()};
