...
(function (Scratch) {
  ...
  class ClonesPlus {
    ...
    stopScriptsInSprite(args) {
      const targetObj = runtime.getSpriteTargetByName(args.INPUT);
      if (targetObj) {
        runtime.stopForTarget(targetObj);
      }
    }

    stopScriptsInMainSprite(args, util) {
      runtime.stopForTarget(util.target.sprite.clones[0]);
    }
    ...
    stopScriptsInClone(args, util) {
      ...
      for (let index = 1; index < clones.length; index++) {
        ...
        if (
          ...
          runtime.stopForTarget(clones[index]);
        }
      }
    }

    deleteClonesInSprite(args, util) {
      const target = runtime.getSpriteTargetByName(args.INPUT);
      if (!target) {
        return;
      }
      const clones = target.sprite.clones;
      for (let index = clones.length - 1; index > 0; index--) {
        runtime.disposeTarget(clones[index]);
      }
    }
    ...
    getSpriteObj(name) {
      ...
      const spriteObj = runtime.getSpriteTargetByName(name);
