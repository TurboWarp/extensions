...
(function (Scratch) {
  ...
  if (!runtime.extensionStorage || Scratch.extensions.isPenguinMod) return alert('The VM is outdated!');
  ...
  async function getWebhookURL(name) {
    ...
    if (!checkValid(webhooks, name)) return alert('A webhook named "'+name+'" already exists.');
  ...
  function createNewEmbed(name) {
    if (!checkValid(embeds, name)) return alert('An embed named "'+name+'" already exists.');
    embeds[uid()] = {
      name: name,
      title: "Title",
