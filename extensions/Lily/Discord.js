...
(function (Scratch) {
  ...
  async function getWebhookName() {
    ScratchBlocks.prompt('Enter webhook name', '', (text) => {
      ...
      getWebhookURL(text);
    }, 'New Webhook', 'broadcast_msg');
  }

  async function getWebhookURL(name) {
    await new Promise((resolveInner) => { setTimeout(resolveInner, 100)});
    ScratchBlocks.prompt('Enter webhook URL', '', (text) => {
      if (!text) return;
  ...
  function createNewEmbed() {
    let name = "";
    ScratchBlocks.prompt('Enter embed name', '', (text) => {
      if (!text) return;
      if (!!embeds[text]) return alert('An embed named "'+text+'"already exists.');
      name = text;
    }, 'New Embed', 'broadcast_msg');
