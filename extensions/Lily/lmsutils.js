...
(function (Scratch) {
  ...
  class LMSUtils {
    ...
    lettersToOf(args) {
      var string = Cast.toString(args.STRING);
    ...
    stringToUpperCase(args) {
      return Cast.toString(args.STRING).toUpperCase();
    }

    stringToLowerCase(args) {
      return Cast.toString(args.STRING).toLowerCase();
    }

    reverseString(args) {
      var input = Cast.toString(args.STRING);
    ...
    angleReporter(args) {
      return Cast.toNumber(args.ANGLE);
    }
    ...
    notEqualTo(args) {
      return Cast.compare(args.INPUTA, args.INPUTB) !== 0;
    }
    ...
    decodeFromBlock(args) {
      if (args.STRING === "") return "";
      if (args.DROPDOWN === "base64") return atob(Cast.toString(args.STRING));
      if (args.DROPDOWN === "binary") {
        var output = Cast.toString(args.STRING);
        return output
          .split(" ")
    ...
    negativeReporter(args) {
      return Cast.toNumber(args.INPUT) * -1;
    }

    exponentBlock(args) {
      return Math.pow(Cast.toNumber(args.INPUTA), Cast.toNumber(args.INPUTB));
    }
    ...
    normaliseValue(args) {
      var input1 = Cast.toNumber(args.INPUT);
    ...
    clampNumber(args) {
      var input1 = Cast.toNumber(args.INPUTA);
      var input2 = Cast.toNumber(args.INPUTB);
      var input3 = Cast.toNumber(args.INPUTC);
    ...
    setSpriteSVG(args, util) {
      try {
        ...
        renderer.updateSVGSkin(skinId, Cast.toString(args.INPUTB));
