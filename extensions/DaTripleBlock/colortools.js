...
(function (Scratch) {
    ...
    class colorTools {
        ...
        decimalhex(args) {
            ...
            return '#' + Scratch.Cast.toString(decimal).split("").map(v => (hex[Scratch.Cast.toNumber(v)])).join("");
        }
        hexdecimal(args) {
            const hex = Scratch.Cast.toString(args.Hex);
            return hex.split("").map(v => (v.charCodeAt(0))).join("");
        }
        decimaltohsv(args) {
        ...
        hsvtodecimal(args) {
            ...
            return hsv.split("").map(v => (v.charCodeAt(0))).join("");
        }
        decimaltorgb(args) {
            const hex = Scratch.Cast.toNumber(args.Hex);
            return `rgb(${hex})`;
        ...
        randomcolor() {
            ...
            return '#' + [1, 1, 1, 1, 1, 1].map(() => (hex[Math.round(Math.random() * hex.length)])).join("");
        }
        wordtocolor(args) {
            switch (args.HSV) {
                case 'red':
