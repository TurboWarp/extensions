...
(function (Scratch) {
  ...
  displaceSrCs = displaceSrCs.map(
    (i) => `data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAA${i}`
  );
  ...
  const tryPure = (string, id, blockID, container) => {
    ...
    if (
      filterInput.startsWith("SPspriteEffects_") &&
      !filterInput.includes("applyCustom")
    ) {
  ...
  */
  ...
  const reImpFilter = (_, observer) => {
    ...
    const thisCanv = supportM
      ? render.canvas
      : render.canvas.parentNode.parentNode.parentNode;
    let string = "";
    for (let i = 0; i < allFilters.length; i++)
      string += `url("#${allFilters[i]}") `;
  ...
  const reqPackagedRAM = () => {
    if (runtime.isPackaged) {
      alert(
        `For the Sprite Effects Extension to work, the creator of the packaged project must uncheck "Remove raw asset data after loading to save RAM" under advanced settings in the packager.`
      );
      return false;
    }
    return true;
  ...
  const genDualBlock = (json) => {
    ...
    imgCopy.opcode = imgCopy.opcode
      .replace("Sprite", "Image")
      .replace("sprite", "image")
      .replace("SPR", "IMG");
    if (!imgCopy.opcode.includes("mage") && !imgCopy.opcode.includes("IMG"))
      imgCopy.opcode += "2";
  ...
  class SPspriteEffects {
      ...
      return {
          ...
          ...genDualBlock({
            ...
            arguments: {
              EFFECT: {
                ...
                defaultValue: "custom-svg-filter",
              },
              ID: {
                type: Scratch.ArgumentType.STRING,
        ...
        menus: {
          ...
          AXISES: {
            ...
            items: ["x axis", "y axis", "both axes"],
          },
          SPLITTING: {
            acceptReporters: true,
            items: ["color split", "abberation", "chromatic aberration"],
          },
          EFFECTS: {
            acceptReporters: true,
            items: [
    ...
    sourceSwitch() {
      (sprite = sprite ? false : true), vm.extensionManager.refreshBlocks();
    }
    toggleDistScale(args) {
      distortionScale = args.ON_OFF === "on";
    }
    toggleCompat(args) {
    ...
    getTargets(includeCanvas) {
      ...
      if (includeCanvas)
        spriteNames.push({ text: "Canvas", value: "_canvas_" });
    ...
    async getSVG(input) {
      ...
      const target =
        input === "_stage_"
          ...
          : runtime.getSpriteTargetByName(input);
      if (!target) {
          ...
          img.onload = () => {
            ...
            const svg = `<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" 
              width="${width / 2}" height="${
                height / 2 + 0.001
              }" viewBox="0,0,${width / 2},${height / 2 + 0.001}">
              <g transform="translate(${img.offsetLeft / -2},${
                img.offsetTop / -2
              })"><g fill="none">
              <image transform="scale(0.5,0.5)" width="${width}" height="${
                height + 0.002
              }" 
    ...
    setSpriteEffect(args, util) {
      return this.setMainEffect(args, false, util);
    }
    async setImageEffect(args) {
      return await this.setMainEffect(args, true);
    }
    async setMainEffect(args, isImage, util) {
      ...
      if (args.SPRITE === "_myself_") svg = await this.findAsset(util);
        ...
        svg = isImage
          ? await this.getImage(args.SPRITE)
          : await this.getSVG(args.SPRITE);
      if (svg) {
        let amtIn = cast.toNumber(args.NUM);
        switch (args.EFFECT) {
          case "saturation":
            filter = `<filter id="saturation"><feColorMatrix type="saturate" values="${
              amtIn / 100
            }"></feColorMatrix></filter>`;
            break;
          case "sepia":
            amtIn /= 100;
            filter = `<filter id="sepia"><feColorMatrix type="matrix" values="${
              0.393 * amtIn
            } ${0.769 * amtIn} ${0.189 * amtIn} 0 0 ${0.349 * amtIn} ${
              0.686 * amtIn
            } ${0.168 * amtIn} 0 0 ${0.272 * amtIn} ${0.534 * amtIn} ${
              0.131 * amtIn
            } 0 0 0 0 0 1 0"></feColorMatrix></filter>`;
            break;
          case "amplify":
            amtIn += 100;
            filter = `<filter id="amplify"><feComponentTransfer><feFuncR type="linear" slope="${
              amtIn / 100
            }"></feFuncR><feFuncG type="linear" slope="${
              amtIn / 100
            }"></feFuncG><feFuncB type="linear" slope="${
              amtIn / 100
            }"></feFuncB></feComponentTransfer></filter>`;
            break;
          case "inflate":
            filter = `<filter id="inflate"><feMorphology in="SourceGraphic" operator="dilate" radius="${Math.abs(
              amtIn / 10
            )}"></feMorphology></filter>`;
            break;
          case "contrast":
            amtIn += 100;
            filter = `<filter id="contrast"><feComponentTransfer><feFuncR type="gamma" exponent="${
              amtIn / 100
            }" amplitude="${
              amtIn / 100
            }" offset="0" /><feFuncG type="gamma" exponent="${
              amtIn / 100
            }" amplitude="${
              amtIn / 100
            }" offset="0" /><feFuncB type="gamma" exponent="${
              amtIn / 100
            }" amplitude="${
              amtIn / 100
            ...
            break;
          case "discrete":
            filter = `<filter id="discrete"><feComponentTransfer><feFuncR type="discrete" tableValues="0 ${
              amtIn / 100
            } 0 1"/><feFuncG type="discrete" tableValues="0 ${
              amtIn / 100
            } 0 1"/><feFuncB type="discrete" tableValues="0 ${
              amtIn / 100
            } 0 1"/><feFuncA type="discrete" tableValues="0 ${
              amtIn / 100
            } 0 1"/></feComponentTransfer></filter>`;
            break;
          case "torn":
            filter = `<filter id="torn"><feOffset in="SourceGraphic" dx="${
              amtIn / -4
            }" dy="${
              amtIn / -4
            }" result="offsetRes" /><feTurbulence type="turbulence" baseFrequency="0.05" numOctaves="2" result="turbulence" /><feDisplacementMap in2="turbulence" in="offsetRes" scale="${amtIn}" xChannelSelector="R" yChannelSelector="G" /></filter>`;
            break;
          case "bevel":
            filter = `<filter id="bevel"><feGaussianBlur stdDeviation="${Math.abs(
              args.NUM / 10
            )}" in="SourceAlpha" result="BLUR"/><feSpecularLighting surfaceScale="2" specularConstant="2" specularExponent="20" lighting-color="#ffffff" in="BLUR" result="SPECULAR"><fePointLight x="40" y="-30" z="50" /></feSpecularLighting><feComposite operator="in" in="SPECULAR" in2="SourceAlpha" result="COMPOSITE"/><feMerge><feMergeNode in="SourceGraphic" /><feMergeNode in="COMPOSITE"/></feMerge></filter>`;
            break;
          case "liquify":
            filter = `<filter id="liquify" width="150%" height="160%" x="-25%" y="-25%"><feTurbulence baseFrequency="0.05" type="fractalNoise" numOctaves="1" seed="${Math.round(
              Math.abs(Math.sin(Date.now() / 5) * 10)
            )}" result="BOTTOM-SPLASH_10"></feTurbulence><feGaussianBlur stdDeviation="${amtIn}" in="SourceGraphic" result="BOTTOM-SPLASH_20"></feGaussianBlur><feDisplacementMap scale="${
              amtIn * 10
            }" in="BOTTOM-SPLASH_20" in2="BOTTOM-SPLASH_10" result="BOTTOM-SPLASH_30"></feDisplacementMap><feComposite operator="in" in="BOTTOM-SPLASH_30" in2="SourceGraphic" result="BOTTOM-SPLASH_40"></feComposite><feTurbulence baseFrequency="0.1" type="fractalNoise" numOctaves="1" seed="${Math.round(
              Math.abs(Math.sin(Date.now() / 5) * 10)
            )}" result="MIDDLE-SPLASH_10"></feTurbulence><feGaussianBlur in="SourceGraphic" stdDeviation="${
              amtIn * 0.01
            }" result="MIDDLE-SPLASH_20"></feGaussianBlur><feDisplacementMap in="MIDDLE-SPLASH_20" in2="MIDDLE-SPLASH_10" scale="${
              amtIn * 2.5
            }" result="MIDDLE-SPLASH_30"></feDisplacementMap><feComposite in="MIDDLE-SPLASH_30" in2="SourceGraphic" operator="in" result="MIDDLE-SPLASH_40"></feComposite><feTurbulence baseFrequency="0.07" type="fractalNoise" numOctaves="1" seed="${Math.round(
              Math.abs(Math.sin(Date.now() / 5) * 10)
            )}" result="TOP-SPLASH_10"></feTurbulence><feGaussianBlur stdDeviation="${
              amtIn * 0.035
            }" in="SourceGraphic" result="TOP-SPLASH_20"></feGaussianBlur><feDisplacementMap scale="${
              amtIn * 22
            }" in="TOP-SPLASH_20" in2="TOP-SPLASH_10" result="TOP-SPLASH_30"></feDisplacementMap><feComposite operator="in" in="TOP-SPLASH_30" in2="SourceGraphic" result="TOP-SPLASH_40"></feComposite></filter>`;
            break;
          case "ripple":
          ...
          case "thermal":
            tableValue = [
              `0 0.125 ${0.8 * (amtIn / 10)} ${1 * (amtIn / 10)} ${
                1 * (amtIn / 10)
              }`,
              `0 0 0 ${0.843 * (amtIn / 10)} 1`,
              `0 ${0.549 * (amtIn / 10)} ${0.466 * (amtIn / 10)} 0 1`,
            ];
            ...
            break;
          case "erode":
            filter = `<filter id="erode"><feMorphology in="SourceGraphic" operator="erode" radius="${Math.abs(
              amtIn / 10
            )}"></feMorphology></filter>`;
            break;
          case "disolve":
            ...
            break;
          case "posterize":
            filter = `<filter id="posterize"><feComponentTransfer><feFuncR type="discrete" tableValues="0 ${
              amtIn / 100
            } 1" /><feFuncG type="discrete" tableValues="0 ${
              amtIn / 100
            } 1" /><feFuncB type="discrete" tableValues="0 ${
              amtIn / 100
            } 1" /></feComponentTransfer></filter>`;
            break;
          case "displacement":
            amtIn = amtIn === 64 ? 65 : amtIn;
            filter = `<filter id="displacement"><feTurbulence type="fractalNoise" baseFrequency="${
              (1000000 - Math.abs(amtIn - 100) * 10000) / 100
            }" numOctaves="2" seed="${Math.round(
              Math.random() * 100
            )}" result="turbulence" /><feDisplacementMap in="SourceGraphic" in2="turbulence" scale="25" xChannelSelector="R" yChannelSelector="G" /></filter>`;
            break;
          case "grain":
            if (amtIn === 0) return svg;
            filter = `<filter id="grain"><feTurbulence type="fractalNoise" baseFrequency="${
              amtIn / 100
            }" numOctaves="3" result="turbulence" seed="${Math.round(
              10 * Math.sin(Date.now() * 50)
            )}" /><feColorMatrix in="turbulence" type="saturate" values="0" result="grain" /><feComposite in="grain" in2="SourceGraphic" operator="in" /><feComposite in="0.5" in2="SourceGraphic" operator="over" /></filter>`;
            break;
          case "bloom":
            filter = `<filter id="bloom"><feGaussianBlur in="SourceGraphic" stdDeviation="5" /><feComposite in2="SourceGraphic" operator="arithmetic" k2="${
              amtIn / 100
            }" k3="1" /></filter>`;
            break;
          case "bloom v2":
            filter = `<filter id="bloom-v2"><feGaussianBlur in="SourceGraphic" stdDeviation="5" /><feComposite in2="SourceGraphic" operator="arithmetic" k2="${
              amtIn / 100
            }" k3="1" color-interpolation-filters="sRGB" /></filter>`;
            break;
          default:
            filter = `<filter id="blur"><feGaussianBlur stdDeviation="${Math.abs(
              amtIn
            )}" in="SourceGraphic" result="BLUR"></feGaussianBlur></filter>`;
        }
        return this.filterApplier(
          svg,
          filter,
          cast.toString(args.EFFECT).replaceAll(" ", "-")
        );
      }
      return svg;
    }

    setSpriteMotion(args, util) {
      return this.motionBlur(args, false, util);
    }
    async setImageMotion(args) {
      return await this.motionBlur(args, true);
    }
    async motionBlur(args, isImage, util) {
      ...
      if (args.SPRITE === "_myself_") svg = await this.findAsset(util);
        ...
        svg = isImage
          ...
          : await this.getSVG(args.SPRITE);
      if (svg) {
        const filter = `<filter id="motionBlur"><feGaussianBlur stdDeviation="${Math.abs(
          args.X
        )}, ${Math.abs(
          args.Y
        )}" in="SourceGraphic" result="BLUR"></feGaussianBlur></filter>`;
    ...
    setSpriteBlend(args, util) {
      return this.blendType(args, false, util);
    }
    async setImageBlend(args) {
      return await this.blendType(args, true);
    }
    ...
    setSpriteVintage(args, util) {
      return this.vintageBlur(args, false, util);
    }
    async setImageVintage(args) {
      return await this.vintageBlur(args, true);
    }
    ...
    applyCustomSprite(args, util) {
      return this.customFilter(args, false, util);
    }
    async applyCustomImage(args) {
      return await this.customFilter(args, true);
    }
    ...
    hueSprite(args, util) {
      return this.setHue(args, false, util);
    }
    async hueImage(args) {
      return await this.setHue(args, true);
    }
    async setHue(args, isImage, util) {
      ...
      if (args.SPRITE === "_myself_") svg = await this.findAsset(util);
        ...
        svg = isImage
          ...
          : await this.getSVG(args.SPRITE);
      if (svg) {
        const filter = `<filter id="hue"><feColorMatrix in="SourceGraphic" type="matrix" values="${this.hexMap(
          args.COLOR
        )}"></feColorMatrix></filter>`;
    ...
    spriteShadow(args, util) {
      return this.addShadow(args, false, util);
    }
    async imageShadow(args) {
      return await this.addShadow(args, true);
    }
    async addShadow(args, isImage, util) {
      ...
      if (svg) {
        ...
        const filter = `<filter id="shadow"><feDropShadow dx="${args.X}" dy="${
          args.Y * -1
        }" stdDeviation="${args.NUM / 2}" flood-color="rgb(${rgbColor.r}, ${
          rgbColor.g
        }, ${rgbColor.b})"/></filter>`;
    ...
    outlineSprite(args, util) {
      return this.addOutline(args, false, util);
    }
    async outlineImage(args) {
      return await this.addOutline(args, true);
    }
    async addOutline(args, isImage, util) {
      ...
      if (svg) {
        ...
        if (args.OUTLINE === "filled") {
          if (supportM)
            filter = `<filter id="filled-outline"><feMorphology operator="erode" radius="${args.NUM}" in="SourceAlpha" result="thickened" /><feFlood flood-color="${args.COLOR}" result="flood"/><feComposite operator="xor" in="flood" in2="thickened" result="frame"/><feComposite operator="atop" in="frame" in2="SourceGraphic"/></filter>`;
          else
            filter = `<filter id="filled-outline"><feMorphology operator="dilate" radius="${args.NUM}" in="SourceAlpha" result="thickened" /><feFlood flood-color="${args.COLOR}" result="flood"/><feComposite operator="in" in="flood" in2="thickened"/><feComposite operator="over" in="SourceGraphic" /></filter>`;
    ...
    splitSprite(args, util) {
      return this.colorSplit(args, false, util);
    }
    async splitImage(args) {
      return await this.colorSplit(args, true);
    }
    async colorSplit(args, isImage, util) {
      ...
      if (svg) {
        if (args.EFFECT.includes("color")) {
          filter = `<filter id="color-split"><feFlood flood-color="${
            args.COLOR1
            ...
            args.X * -1
          }" dy="${args.Y}" result="RED_OFF"/><feOffset in="GREEN" dx="${
            args.X
          }" dy="${
            args.Y * -1
          ...
          filter = `<filter id="chromatic-abberation"><feOffset in="SourceGraphic" dx="${
            args.X
          }" dy="${
            args.Y
          }" result="layer-one" /><feColorMatrix in="layer-one" result="customColor1" type="matrix" values="${this.hexMap(
            args.COLOR1
          )}"></feColorMatrix><feOffset in="SourceGraphic" dx="${
            args.X * -1
          }" dy="${
            args.Y * -1
          }" result="layer-two" /><feColorMatrix in="layer-two" result="customColor2" type="matrix" values="${this.hexMap(
            args.COLOR2
          ...
          filter = `<filter id="abberation"><feFlood flood-color="${
            args.COLOR1
          }" flood-opacity="0.5" result="RED"/><feFlood flood-color="${
            args.COLOR2
    ...
    waveSprite(args, util) {
      return this.waveEffect(args, false, util);
    }
    async waveImage(args) {
      return await this.waveEffect(args, true);
    }
    async waveEffect(args, isImage, util) {
      ...
      if (svg) {
        const channel = [
          args.axis === "x axis" ? "A" : "R",
          args.axis === "y axis" ? "A" : "G",
        ];
        const filter = `<filter id="wave"><feTurbulence type="fractalNoise" baseFrequency="${
          args.Y / 100
        }, ${args.NUM / 100}" numOctaves="2" seed="${
          args.SEED
        }" result="turbulence" /><feDisplacementMap in="SourceGraphic" in2="turbulence" scale="25" xChannelSelector="${
          channel[0]
        }" yChannelSelector="${channel[1]}" /></filter>`;
        return this.filterApplier(svg, filter, "wave");
      }
      return svg;
    }

    tileSprite(args, util) {
      return this.addTile(args, false, util);
    }
    async tileImage(args) {
      return await this.addTile(args, true);
    }
    async addTile(args, isImage, util) {
      ...
      if (svg) {
        ...
        atts = [
          cast.toNumber(
            atts[0] ? parseFloat(atts[0][1]) : render.canvas.width / 2
          ),
          cast.toNumber(
            atts[1] ? parseFloat(atts[1][1]) : render.canvas.height / 2
          ),
        ];
        const amts = [
          (canvasMode
            ? (100 - Math.abs(cast.toNumber(args.NUM))) / 100
            : Math.abs(cast.toNumber(args.NUM)) / 100) *
            (atts[0] / 2),
        ...
        let filter;
        if (canvasMode)
          filter = `<filter id="tile"><feOffset dx="0" dy="0" x="${amts[1]}%" y="${amts[2]}%" width="${amts[0]}%" height="${amts[0]}%" in="tile" result="off"/><feTile x="0" y="0" width="${atts[0]}" height="${atts[1]}" in="off"/></filter>`;
        else
          filter = `<filter id="tile"><feImage x="${amts[1]}" y="${
            amts[2]
          }" width="${atts[0] / amts[0]}%" height="${
            atts[0] / amts[0]
          }%" xlink:href="${await this.svgToBitmap(
            svg,
            atts[0],
            atts[1]
          )}"/><feTile /></filter>`;
    ...
    patternSprite(args, util) {
      return this.addPattern(args, false, util);
    }
    async patternImage(args) {
      return await this.addPattern(args, true);
    }
    async addPattern(args, isImage, util) {
      ...
      if (svg && svg2) {
        ...
        const pattern = `<defs><pattern id="${name}" patternUnits="userSpaceOnUse" width="${
          atts[0]
        }" height="${atts[1]}" patternTransform="scale(${
          atts[2]
        })"><image xlink:href="${await this.svgToBitmap(
          svg2,
          atts[0],
          atts[1]
        )}" width="${atts[0]}" height="${atts[1]}" /></pattern></defs>`;
        const svgTag = svg.indexOf(">");
        return `${svg.substring(0, svgTag + 1)}${pattern}${svg.substring(
          svgTag + 1,
          svg.length
        )}`.replaceAll(args.COLOR, `url(#${name})`);
      }
      return svg;
    }

    applySpriteLight(args, util) {
      return this.lighting(args, false, util);
    }
    async applyImageLight(args) {
      return await this.lighting(args, true);
    }
      ...
      if (svg) {
        ...
        const pos = [
          cast.toNumber(args.X) +
            cast.toNumber(
              boxMatch[0]
                ? parseFloat(boxMatch[0][1]) / 2
                : runtime.stageWidth / 2
            ),
          cast.toNumber(args.Y) * -1 +
            cast.toNumber(
              boxMatch[1]
                ? parseFloat(boxMatch[1][1]) / 2
                : runtime.stageHeight / 2
            ),
        ];
        const off =
          args.SPRITE === "_canvas_"
            ? canvas.getBoundingClientRect().width / runtime.stageWidth
            : 1;
        const filter = `<filter id="lighting"><feSpecularLighting result="specOut" specularExponent="20" lighting-color="${
          args.COLOR
        }"><fePointLight x="${pos[0] * off}" y="${pos[1] * off}" z="${
          cast.toNumber(args.NUM) * off
        }" /></feSpecularLighting><feComposite in="SourceGraphic" in2="specOut" operator="arithmetic" k1="0" k2="1" k3="1" k4="0" /></filter>`;
        return this.filterApplier(svg, filter, "lighting");
      }
      return svg;
    }

    advSpriteLight(args, util) {
      return this.advLighting(args, false, util);
    }
    async advImageLight(args) {
      return await this.advLighting(args, true);
    }
    async advLighting(args, isImage, util) {
      ...
      if (args.SPRITE === "_myself_") svg = await this.findAsset(util);
        ...
        svg = isImage
          ...
          : await this.getSVG(args.SPRITE);
      const source =
        args.MAP.startsWith("data:image/") || args.MAP.startsWith("http")
          ? args.MAP
          : `data:image/svg+xml;base64,${btoa(args.MAP)}`;
      if (svg) {
        const boxMatch = [
          /width="([^"]*)"/.exec(svg),
          /height="([^"]*)"/.exec(svg),
        ...
        const size = [
          cast.toNumber(
            boxMatch[0]
              ? parseFloat(boxMatch[0][1])
              : parseFloat(canvasBox.width)
          ),
          cast.toNumber(
            boxMatch[1]
              ? parseFloat(boxMatch[1][1])
              : parseFloat(canvasBox.height)
          ),
        ];
    ...
    distortSprite(args, util) {
      return this.setDistort(args, false, false, util);
    }
    async distortImage(args) {
      return await this.setDistort(args, true, false);
    }
    distortSpriteImage(args, util) {
      return this.setDistort(args, false, true, util);
    }
    async distortImageImage(args) {
      return await this.setDistort(args, true, true);
    }
    async setDistort(args, isImage, override, util) {
      ...
      if (svg) {
        let source,
          tableValue,
          effect = cast.toString(args.EFFECT);
        if (override)
          source = effect.startsWith("data:image/")
            ? effect
            : `data:image/svg+xml;base64,${btoa(effect)}`;
        else
          source =
            effect === "bulge"
              ? 0
        ...
        const mul = isCanvas
          ? parseFloat(computed.width) / runtime.stageWidth
          : 1;
        const amts = [
          cast.toNumber(args.NUM),
          cast.toNumber(args.X) * mul,
          cast.toNumber(args.Y) * mul,
        ];
        if (isCanvas) {
          tableValue = [
          ...
          if (typeof scaffolding !== "undefined")
            amts[1] +=
              tableValue[0] / (1 + render.canvas.width / runtime.stageWidth);
        } else {
          tableValue = [
            /width="([^"]*)"/.exec(svg),
            /height="([^"]*)"/.exec(svg),
          ...
          if (tableValue[0] === null || tableValue[1] === null) return svg;
            ...
            tableValue = [
              parseFloat(tableValue[0][1]),
              parseFloat(tableValue[1][1]),
            ];
        }
        if (distortionScale || effect === "ripple" || effect === "shockwave") {
          const value = amts[0] / 100;
          const og = [...tableValue];
          tableValue[0] *= 1 - value;
          tableValue[1] *= 1 - value;
          amts[1] += (og[0] - tableValue[0]) / 2;
          amts[2] -= (og[1] - tableValue[1]) / 2;
        }
        const filter = `<filter id="${
          override ? "customDistort" : effect
        }" xmlns:xlink="http://www.w3.org/1999/xlink">
          <feImage id="dMapS" xlink:href="${
            override ? source : displaceSrCs[source]
          }" x="${amts[1]}" y="${-amts[2]}" width="${Math.abs(
            tableValue[0]
          )}" height="${Math.abs(tableValue[1])}" result="distortImg" />
          <feDisplacementMap id="dMapRes" xChannelSelector="R" yChannelSelector="G" in="SourceGraphic" in2="distortImg" result="displacementMap" color-interpolation-filters="sRGB" scale="${
            amts[0] * mul
          }" /><feComposite operator="in" in2="distortImg"></feComposite>
        ${
          amts[0] < 0 && effect !== "ripple"
            ? ""
    ...
    distortPreset(args) {
      return (
        displaceSrCs[
          { bulge: 0, whirl: 1, ripple: 2, shockwave: 3 }[args.TYPE]
        ] || ""
      );
    }

    glitchSprite(args, util) {
      return this.setGlitch(args, false, util);
    }
    async glitchImage(args) {
      return await this.setGlitch(args, true);
    }
    async setGlitch(args, isImage, util) {
      ...
      if (svg) {
        ...
        for (let i = 1; i < args.LINE; i++) {
          ...
          mergeTxt += `<feOffset in="SourceGraphic" dx="${
            args.X * (Math.random() > 0.5 ? 1 : -1)
          }" dy="${
            args.Y * (Math.random() > 0.5 ? 1 : -1)
          }" result="off${i}"/><feMerge x="${
            axis[0] ? Math.random() * mul : 0
          }%" y="${axis[1] ? Math.random() * mul : 0}%" width="${
            axis[0] ? args.NUM : mul
          }%" height="${
            axis[1] ? args.NUM : mul
          }%" result="merge${i}"><feMergeNode in="SourceGraphic" /><feMergeNode in="off${i}" /></feMerge>`;
        }
    ...
    vhsSprite(args, util) {
      return this.setVHS(args, false, util);
    }
    async vhsImage(args) {
      return await this.setVHS(args, true);
    }
    async setVHS(args, isImage, util) {
      ...
      if (svg) {
        const full =
          render.canvas.getBoundingClientRect().width > runtime.stageWidth
            ? 1
            : 1.9;
        const mul =
          args.SPRITE === "_canvas_"
            ? [runtime.stageHeight / full, runtime.stageWidth / full]
            : [100, 100];
        const axis =
          args.axis === "x axis"
            ? [true, false]
        ...
        const filter = `<filter id="vhs"><feOffset in="SourceGraphic" dx="${
          amts[0]
        }" dy="${amts[1]}" result="off"/><feMerge in="SourceGraphic" x="${
          axis[0] ? (100 - amts[2]) * (mul[0] / 100) : 0
        }%" y="${axis[1] ? (100 - amts[2]) * (mul[1] / 100) : 0}%" width="${
          axis[0] ? amts[2] * mul[0] : mul[0]
        }%" height="${
          axis[1] ? amts[2] * mul[1] : mul[1]
        }%" result="merge1"><feMergeNode in="off" /><feMergeNode in="merge1" /></feMerge><feMerge in="off" x="0%" y="0%" width="${
          axis[0] ? mul[0] - amts[2] : mul[0]
        }%" height="${
          axis[1] ? mul[1] - amts[2] : mul[1]
        }%" result="merge2"><feMergeNode in="SourceGraphic" /><feMergeNode in="merge2" /></feMerge><feMerge>${
          amts[2] < 100
            ? `<feMergeNode in="merge1" /><feMergeNode in="merge2" />`
    ...
    ditherSprite(args, util) {
      return this.dither(args, false, util);
    }
    async ditherImage(args) {
      return await this.dither(args, true);
    }
    ...
    maskSprite(args, util) {
      return this.mask(args, false, util);
    }
    async maskImage(args) {
      return await this.mask(args, true);
    }
    async mask(args, isImage, util) {
      ...
      if (args.SPRITE2 === "_myself_") svg2 = await this.findAsset(util);
        ...
        svg2 = isImage
          ? await this.getImage(args.SPRITE2)
          : await this.getSVG(args.SPRITE2);
      const multiply =
        args.SPRITE === "_canvas_"
          ? [
              render.canvas.width * 2,
              render.canvas.height * 2,
              render.canvas.width / runtime.stageWidth,
            ]
          : [100, 100, 1];
      if (svg && svg2) {
        const name = `mask${args.TYPE}`.trim();
        const filter = `<filter id="${name}">
        	<feImage xlink:href="${`data:image/svg+xml;base64,${btoa(svg2)}`}" x="${
            maskOptions[0] * multiply[2]
          }" y="${maskOptions[1] * multiply[2]}" width="${
            maskOptions[2] * multiply[2]
          }%" height="${
            maskOptions[2] * multiply[2]
          }%" preserveAspectRatio="xMidYMid meet" crossOrigin="anonymous" result="image1"/>
        	<feComposite in="${
            args.TYPE !== " " ? "SourceGraphic" : "image1"
          }" in2="${
            args.TYPE !== " " ? "image1" : "SourceGraphic"
          }" operator="${args.TYPE}" x="0%" y="0%" width="${
            multiply[0]
          }%" height="${multiply[1]}%" result="composite1"/></filter>`;
    ...
    setMaskSZ(args) {
      maskOptions[2] = cast.toNumber(args.SIZE);
    }
    maskATT(args) {
      return maskOptions[args.ATT];
    }

    unClipSPR(args, util) {
      return this.updateView(args, false, util);
    }
    async unClipIMG(args) {
      return await this.updateView(args, true);
    }
    async updateView(args, isImage, util) {
      ...
      if (svg) {
        ...
        if (viewboxMatch)
          viewboxVals = viewboxMatch[1].split(/\s*,\s*/).map(parseFloat);
        ...
        vals = `${viewboxVals},${translateVals}`
          .split(",")
          .map((item) => cast.toNumber(item));
        const num = cast.toNumber(args.NUM);
        if (vals.length > 3) {
          svg = svg
            .replace(
              /viewBox="([^"]+)"/,
              `viewBox="${vals[0]},${vals[1]},${vals[2] + num * 2},${
                vals[3] + num * 2
              }"`
            )
            ...
            .replace(
              /<g transform="([^"]+)"/,
              `<g transform="translate(${vals[4] + num},${vals[5] + num})"`
            );
        }
      }
      return svg;
    ...
    setXY(args, util) {
      return this.setATT(args, false, "XY", util);
    }
    async setXY2(args) {
      return await this.setATT(args, true, "XY");
    }
    setDIR(args, util) {
      return this.setATT(args, false, "DIR", util);
    }
    async setDIR2(args) {
      return await this.setATT(args, true, "DIR");
    }
    setSCALE(args, util) {
      return this.setATT(args, false, "SZ", util);
    }
    async setSCALE2(args) {
      return await this.setATT(args, true, "SZ");
    }
    setSKEW(args, util) {
      return this.setATT(args, false, "SKEW", util);
    }
    async setSKEW2(args) {
      return await this.setATT(args, true, "SKEW");
    }
    async setATT(args, isImage, type, util) {
      ...
      if (svg) {
        ...
        if (type === "XY")
          return svg.replace(
            /translate\([^)]*\)/,
            `translate(${currentX + x * 100},${currentY + y * -100})`
          );
        else if (type === "DIR") {
          if (con)
            svg = svg.replace(
              /(style="[^"]*transform:[^"]*)/,
              `$1 rotate(${cast.toNumber(args.DIR) - 90}deg)`
            );
          else
            svg = svg.replace(
              `width="${width}" height="${height}"`,
              `width="${width}" height="${height}" style="transform-origin: center; transform: rotate(${
                cast.toNumber(args.DIR) - 90
              }deg)"`
            );
        } else if (type === "SZ") {
          let vals = "";
          ...
          if (vals.length > 1) {
            svg = svg
              .replace(
                `width="${width}" height="${height}"`,
                `width="${width * Math.abs(x)}" height="${
                  height * Math.abs(y)
                }"`
              )
              .replace(
                /viewBox="([^"]+)"/,
                `viewBox="${vals[0]},${vals[1]},${width * Math.abs(x)},${
                  height * Math.abs(y)
                }"`
              )
              .replace(
                `(${currentX},`,
                `(${currentX * x + (args.x < 0 ? width * Math.abs(x) : 0)},`
              )
              .replace(
                `,${currentY})`,
                `,${currentY * y + (args.y < 0 ? height * Math.abs(y) : 0)})`
              );
            transform = `scale(${x}, ${y})`;
          }
        } else {
          if (con)
            svg = svg.replace(
              /(style="[^"]*transform:[^"]*)/,
              `$1 skew(${args.x}deg, ${args.y}deg)`
            );
          else
            svg = svg.replace(
              `width="${width}" height="${height}"`,
              `width="${width}" height="${height}" style="transform-origin: center; transform: skew(${args.x}deg, ${args.y}deg)"`
            );
          return svg;
        }
        ...
        return svg.replace(
          /transform="([^"]*)"/,
          `transform="${
            curTransform ? `${curTransform} ${transform}` : transform
          }"`
        );
      }
      return svg;
    ...
    addCanvasFilter(args, util) {
      ...
      if (match && match[1]) {
        ...
        const name = cast
          ...
          .replaceAll('"', "-'")
          .replaceAll(" ", "_");
        const oldFilter = document.querySelector(
          `div[id^="SP-canvas-${name}"]`
        );
        if (oldFilter) {
        ...
        if (!curFilter.includes(`url(#${name})`))
          canvas.style.filter = curFilter
            ? `${curFilter} url(#${name})`
            : `url(#${name})`;
      } else {
        console.warn("Invalid Filter, Cancelled Application");
      }
    }
    ...
    removeCanvasFilter(args) {
      ...
      if (curFilter.includes(`url("#${name}")`)) {
        ...
        console.warn("Filter not found, Cancelled Deletion");
      }
    }
