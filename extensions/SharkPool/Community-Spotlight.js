...
(function (Scratch) {
  ...
  if (!Scratch.extensions.unsandboxed)
    throw new Error("Community Spotlight must run unsandboxed!");
  ...
  const acceptableImgs = [
    ...
    "300x250 Rectangle",
    "480x270 Widescreen",
    "300x50 Horizontal Banner",
    "50x300 Vertical Banner",
    "360x120 Large Horizontal Banner",
    "120x360 Large Vertical Banner",
  ];
  ...
  window.CommunitySpotlight = {
    b: "https://raw.githubusercontent.com/Community-Spotlight/",
    u: {},
    c: {},
  };
  window.CommunitySpotlight.u["fetchIndex"] = async function () {
    const r = await fetch(
      `${window.CommunitySpotlight.b}promotion-index/main/index.json`
    );
    if (!r.ok) throw new Error("Couldn't fetch promotions!");
    return await r.json();
  };
  window.CommunitySpotlight.u["filterPromos"] = function (j, t, p) {
    const r = (a) => a[Math.floor(Math.random() * a.length)];
    t = t === "video" ? "video" : t === "html" ? "html" : "image";
    p = typeof p === "object" ? p : {};
    if (p.tags) {
      if (p.tags.constructor.name === "Array") {
        p.tags = p.tags.map((e) => {
    ...
    const { aspectRatio, videoLength } = p;
    let c = Object.values(j).filter((q) => {
      return q.media[t + "s"].length > 0;
    });
    if (c.length === 0) {
    ...
    if (p.tags && p.tags.length > 0)
      c = c.filter((q) => q.tags.some((tag) => p.tags.includes(tag)));
    if (aspectRatio)
      c = c.filter((q) => {
        return q.media[t + "s"].some((i) => {
          return i.size === aspectRatio;
        });
      });
    if (t === "video" && videoLength)
      c = c.filter((q) => {
        return q.media.videos.some((i) => {
          return i.length === videoLength;
        });
      });
    if (c.length === 0) {
    ...
    switch (t) {
      case "video":
        (f = g.media.videos),
          (m =
            aspectRatio && videoLength
              ? f.find(
                  (e) => e.size === aspectRatio && e.length === videoLength
                )
              : !aspectRatio && videoLength
                ? f.find((e) => e.length === videoLength)
                : aspectRatio && !videoLength
                  ? f.find((e) => e.size === aspectRatio)
                  : r(f)),
          (l = `sz${m.size.replace(":", "x")}leng${m.length}.${m.type}`);
        break;
      case "html":
        (f = g.media.htmls),
          (l = `sz${(m = aspectRatio ? f.find((e) => e.size === aspectRatio) : r(f)).size.replace(":", "x")}.html`);
        break;
      default:
        (f = g.media.images),
          (m = aspectRatio ? f.find((e) => e.size === aspectRatio) : r(f)),
          (l = `${m.size}.${m.type}`);
    }
    delete g.media;
    g.url = `${window.CommunitySpotlight.b}promotion-media/main/${encodeURIComponent(g.id)}/${l}`;
    return g;
  };
  async function refreshPromoCacheCS() {
    try {
      window.CommunitySpotlight.c =
        await window.CommunitySpotlight.u["fetchIndex"]();
    } catch (e) {
      console.error(e);
    }
  }
  async function getOnlinePromoCS(t, p) {
    return window.CommunitySpotlight.u["filterPromos"](
      await window.CommunitySpotlight.u["fetchIndex"](),
      t,
      p
    );
  }
  function getCachedPromoCS(t, p) {
    return window.CommunitySpotlight.u["filterPromos"](
      structuredClone(window.CommunitySpotlight.c),
      t,
      p
    );
  }
  ...
  class SPspotlight {
    ...
    filterTags(args) {
      try {
        tags = JSON.parse(Scratch.Cast.toString(args.TAGS).toLowerCase());
      } catch {
        tags = [];
      }
    }
    ...
    getImgPromo(args) {
      ...
      return JSON.stringify(getCachedPromoCS("image", { tags, aspectRatio }));
    }
    showImgPromo(args) {
      if (!initialized) return;
      this.displayPromo(JSON.parse(this.getImgPromo(args)), args.SIZE);
    }
    ...
    getVidPromo(args) {
      ...
      const videoLength =
        args.LENGTH === "any" ? undefined : Scratch.Cast.toNumber(args.LENGTH);
      return JSON.stringify(
        getCachedPromoCS("video", {
          tags,
          videoLength,
          aspectRatio: args.SIZE,
        })
      );
    }
    showVidPromo(args) {
      if (!initialized) return;
      this.displayPromo(JSON.parse(this.getVidPromo(args)), args.SIZE);
    }
    ...
    showHTMLPromo(args) {
      if (!initialized) return;
      this.displayPromo(JSON.parse(this.getHTMLPromo(args)), args.SIZE);
    }
    ...
    setPromoPos(args) {
      this.promoSpaceInfo.pos = [
        ...
        Scratch.Cast.toNumber(args.y) * -1,
      ];
      this.updatePromo();
    }
    ...
    setPromoScale(args) {
      this.promoSpaceInfo.sz = [
        Scratch.Cast.toNumber(args.x) / 100,
    ...
    Scratch.gui.getBlockly().then((SB) => {
      ...
      if (!SB?.SPgradients?.patched) {
        ...
        BSP.render = function (...args) {
          const blockTheme =
            ReduxStore.getState().scratchGui.theme.theme.blocks;
      ...
      ScratchBlocks.SPgradients.gradientUrls["SPspotlight"] = [
        "url(#SPspotlight-GRAD)",
      ];
    });
