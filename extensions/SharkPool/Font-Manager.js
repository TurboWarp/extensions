...
(function (Scratch) {
  ...
  class SPASfontManager {
    ...
    customFonts() {
      return JSON.stringify(manage.fonts);
    }
    ...
    fontExists(args) {
      return manage.hasFont(Scratch.Cast.toString(args.NAME));
    }
    ...
    addSystemFont(args) {
      ...
      if (manage.isValidFamily(args.NAME))
        manage.addSystemFont(args.NAME, args.BACKUP);
    }
    ...
    onChanged() {
      const oldFontNames = this.fontNames(oldFonts),
        fontNames = this.fontNames(structuredClone(manage.fonts));
