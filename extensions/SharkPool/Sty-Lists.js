...
(function (Scratch) {
  ...
  if (typeof scaffolding !== "undefined") {
    listDocs = {
      ...
      value: "sc-monitor-row-value-outer",
      value2: "sc-monitor-row-value-inner",
      ind: "sc-monitor-row-index",
      head: "sc-monitor-list-label",
      foot: "sc-monitor-list-footer-text",
      pkg1: "",
      addButton: "sc-monitor-list-add",
      grid: ".sc-monitor-rows-inner",
      contain: ".sc-monitor-rows-inner",
      row: "sc-monitor-row-root",
      span: "",
    };
  } else {
    listDocs = {
      body: "monitor_list-monitor",
      value: "monitor_list-value",
      value2: "monitor_value-inner",
      ind: "monitor_list-index",
      head: "monitor_list-header",
      foot: "monitor_footer-length_2",
      pkg1: `[class*="monitor_"]`,
      addButton: "monitor_add-button",
      grid: ".ReactVirtualized__Grid.ReactVirtualized__List",
      contain: ".ReactVirtualized__Grid__innerScrollContainer",
      row: "monitor_list-row",
      span: "span",
    };
  }
  let visualLists = {};
  ...
  class SPlistMonitors {
          ...
          if (!monitor) delete visualLists[id];
            ...
            const monBody =
              typeof scaffolding !== "undefined"
                ? monitor
                : monitor.querySelector(`div[class^="${listDocs.body}"]`);
            this.stretchList(
              {
                LIST: id,
                WIDTH: parseFloat(monBody.style.width),
                HEIGHT: parseFloat(monBody.style.height),
                SECRET: true,
              },
              util
            );
            ...
            if (columns > 1)
              this.renderList({ LIST: id, NUM: columns, SECRET: true }, util);
          }
        }
      });
    }
    ...
    getLists() {
      ...
      if (listMenu.length === 0)
        return [{ text: "make a list", value: "make a list" }];
    ...
    getMonitor(id, util, returnID) {
      ...
      if (returnID)
        return {
          id,
          listMon: document.querySelector(
            `div[data-id="${id}"][class*="monitor"]`
          ),
        };
      else
        return document.querySelector(`div[data-id="${id}"][class*="monitor"]`);
    }
    ...
    applyChanges(listMon, id) {
      ...
      const allItems = listMon.querySelectorAll(
        `div[class^="${listDocs.body}"] [class^="${listDocs.value}"]`
      );
      ...
      allItems.forEach((item) => {
        const i = isPackaged
          ...
          : item.getAttribute("dataindex");
        if (newLabels[i]) item.previousSibling.textContent = newLabels[i];
        if (isPackaged)
          item.style.background =
            newColors[i] || ""; // Labels auto reset, colors do not
    ...
    listExists(args, util) {
      return Scratch.Cast.toBoolean(this.getListID(args.LIST, util));
    }
    ...
    makeList(args, util) {
      ...
      if (args.TYPE === "for this sprite only")
        util.target.createVariable(this.genId(), listName, "list");
      else
        runtime
          .getTargetForStage()
          .createVariable(this.genId(), listName, "list");
      return this.refresh();
    }
    ...
    renderList(args, util) {
      ...
      const columnCnt = Math.min(
        100,
        Math.round(Scratch.Cast.toNumber(args.NUM))
      );
      listMon.setAttribute("SPcolumnCnt", columnCnt);
      let container = listMon.querySelector(
        `div[class^="SPstyLists-columnRender"`
      );
      ...
      let itemDiv = [],
        listContain = listMon.querySelector(listDocs.contain);
      ...
      if (columnCnt === 1) listContain.style.display = "";
        ...
        if (args.SECRET === undefined)
          visualLists[id] = { ...visualLists[id], util };
        this.applyChanges(listMon, id);
      }
    }
    ...
    stretchList(args, util) {
      ...
      const width = Math.max(
        1,
        Math.min(99999, Scratch.Cast.toNumber(args.WIDTH))
      );
      const height = Math.max(
        1,
        Math.min(99999, Scratch.Cast.toNumber(args.HEIGHT))
      );
      if (args.SECRET === undefined)
        visualLists[id] = { ...visualLists[id], util };
      if (typeof scaffolding === "undefined") {
        // Editor Monitor GUI cant Force Visual Updates, we need to do it Manually
        const listMon = document.querySelector(
          `div[data-id="${id}"] [class*="monitor_list"]`
        );
        if (!listMon) return;
        const outerModal = listMon.querySelector(listDocs.grid);
        const innerModal = listMon.querySelector(listDocs.contain);
        listMon.style.width = `${width}px`;
        listMon.style.height = `${height}px`;
        outerModal.style.width = `${width}px`;
        outerModal.style.height = `${height}px`;
        if (innerModal) {
    ...
    getAtt(args, util) {
      ...
      listMon =
        typeof scaffolding !== "undefined"
          ? listMon
          : listMon.querySelector(`div[class^="${listDocs.body}"]`);
      return parseFloat(
        listMon.style[args.THING === "width" ? "width" : "height"]
      );
    }
    ...
    chooseOption(args, util) {
      ...
      switch (args.THING) {
        ...
        case "text highlighting": {
          const items = listMon.querySelectorAll(
            `div[class^="${listDocs.value2}"]`
          );
          items.forEach((item) => {
        ...
        case "numbered items": {
          const numberedItems = listMon.querySelectorAll(
            `div[class^="${listDocs.ind}"]`
          );
          numberedItems.forEach((item) => {
            item.style.display = disable ? "none" : "block";
          });
          break;
        }
        case "+ and = buttons": {
          let button1 = listMon.querySelector(
            `div[class^="${listDocs.addButton}"]`
          );
          let button2 =
            typeof scaffolding !== "undefined"
              ? button1
              : listMon.querySelector(`div[class^="monitor_resize-handle"]`);
          if (button1 === null || button2 === null) {
            const buttons = listMon.querySelectorAll(".no-drag");
            buttons.forEach((button) => {
              button.style.display = disable ? "none" : "block";
            });
          } else {
    ...
    setPosition(args, util) {
      ...
      const y =
        (Scratch.Cast.toNumber(args.y) - canvas[1] + sizeOffset[1] / 2) * -1;

      let styleAtt = listMon.getAttribute("style");
      const transformMatch = styleAtt.match(regex);
      if (transformMatch) {
        const newTransform = transformMatch[1].replace(
          /translate\([^)]+\)/,
          `translate(${x}px, ${y}px)`
        );
        listMon.setAttribute(
          "style",
          styleAtt.replace(regex, `transform:${newTransform}; top: 5; left: 5;`)
        );
        runtime.requestUpdateMonitor(
          new Map([
            ["id", id],
    ...
    currentPos(args, util) {
      ...
      if (styleAtt) {
        const match = styleAtt.match(
          /transform\s*:\s*translate\((-?\d+(?:\.\d+)?px),\s*(-?\d+(?:\.\d+)?px)\)/
        );
        if (match)
          return args.POSITION === "x"
            ? parseInt(match[1]) - canvas[0] + sizeOffset[0] / 2
    ...
    setScroll(args, util) {
      ...
      let grid = document.querySelector(
        `div[data-id="${id}"] ${listDocs.grid}`
      );
      let spGrid = document.querySelector(
        `div[data-id="${id}"] [class^="SPstyLists-columnRender"`
      );
      const value = Scratch.Cast.toNumber(args.SCROLL) / 100;
      if (spGrid !== null)
        spGrid.scrollTop = value * (spGrid.scrollHeight - spGrid.clientHeight);
      else if (grid !== null)
        grid.scrollTop = value * (grid.scrollHeight - grid.clientHeight);
    }
    ...
    formatItems(args, util) {
      ...
      const items = Array.from(
        listMon.querySelectorAll(`div[class^="${listDocs.value}"]`)
      );
      items.forEach((item) => {
        item.style.textAlign = args.ALIGN;
      });
    }
    ...
    setDisplay(args, util) {
      ...
      if (args.DISPLAY === "header")
        label = document.querySelector(
          `div[data-id="${id}"] ${listDocs.pkg1}[class^="${listDocs.head}"]`
        );
      else
        label = document.querySelector(
          `div[data-id="${id}"] ${listDocs.pkg1}[class^="${listDocs.foot}"] ${listDocs.span}`
        );
    ...
    setColor(args, util) {
      ...
      visualLists[id] = {
        ...currentList,
        util,
        colors: { ...currentList.colors, [index]: args.COLOR },
      };
      this.applyChanges(listMon, id);
    }
    ...
    setLabel(args, util) {
      ...
      visualLists[id] = {
        ...currentList,
        util,
        labels: { ...currentList.labels, [index]: val },
      };
      this.applyChanges(listMon, id);
    }
    ...
    resetEffect(args, util) {
      ...
      let translation = matches
        ...
        : "";
      listMon.style.transform = translation.replace(
        /scale\([^,]+,\s*[^)]+\)/g,
        "scale(1)"
      );
    ...
    setEffect(args, util) {
      ...
      let effect = args.EFFECT,
        amountIn = Scratch.Cast.toNumber(args.AMOUNT);
      ...
      if (effect === "rotate" || effect.includes("skew")) {
        ...
        curFilter += ` ${effect}(${amountIn}${
          effect === "blur" ? "px" : effect === "hue-rotate" ? "deg" : "%"
        })`;
    ...
    stretchEffect(args, util) {
      ...
      if (!listMon) return;
      let curTransform = listMon.style.transform || "";
      const scaleTxt = `scale(${Scratch.Cast.toNumber(args.x) / 100}, ${
        Scratch.Cast.toNumber(args.y) / 100
      })`;
      listMon.style.transform = `${curTransform.replace(
        /scale\([^)]*\)/g,
        ""
      )} ${scaleTxt}`;
    }
    ...
    currentEffect(args, util) {
      ...
      if (filterMatch || transformMatch) {
        ...
        if (setEffect === "brightness") return pureVal - 100;
          ...
          return unitVal.split(", ")[effect.includes("x") ? 0 : 1] * 100;
        else return pureVal;
      } else {
        return defaultV;
      }
