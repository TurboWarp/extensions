...
void main() {
  ...
  const updateEngine = (engine) => {
    ...
    for (const key in emitters) {
      ...
      const emit =
        maxP + 1 - Math.round(rng(opts.emission.val, opts.emission.inf));
    ...
    getBehaviour(args) {
      ...
      if (target && target[engineTag]) {
        ...
        if (engine.emitters[args.NAME]) {
          ...
          if (opt) {
            if (args.BEHAVE.endsWith("Col") && args.TYPE === "value")
              return JSON.stringify(hsl2Rgb(...Object.values(opt.val)));
  ...
  if (Scratch.gui)
    Scratch.gui.getBlockly().then((SB) => {
      ...
      SB.SPgradients.gradientUrls["SPpartEngine"] = [
        "url(#SPpartEngine-GRAD1)",
      ];
    });
