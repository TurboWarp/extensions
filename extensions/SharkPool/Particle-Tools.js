...
(function (Scratch) {
  ...
  if (!Scratch.extensions.unsandboxed) {
    throw new Error("Particle Tools must run unsandboxed");
  }
  ...
  class ParticleToolsSP {
    ...
    genVel(args) {
      if (args.TorF === "on") {
        ...
        return (
          Math.round((Math.random() * (max - min + 1) + min) * 10000) / 10000
        );
      } else {
        const max = Scratch.Cast.toNumber(args.MAX);
        const min = Scratch.Cast.toNumber(args.MIN);
