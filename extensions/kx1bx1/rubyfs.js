...
(function(Scratch) {
	...
	class RubyFS {
		...
		_internalDirName(path) {
			if (path === "/") return "/";
			let procPath = this._isPathDir(path) ? path.substring(0, path.length - 1) : path;
		...
		clean() {
			this.lastError = "";
			if (!this.hasPermission("/", "delete")) return this._setError("Clean failed: No 'delete' permission on /");
			this._internalClean();
		}
		...
		sync({
			...
			if (path1 === "/") return this._setError("Rename failed: Root directory cannot be renamed");

			if (!this.hasPermission(path1, "delete")) return this._setError("Rename failed: No 'delete' permission");
			if (this.fs.has(path2)) return this._setError("Rename failed: Destination exists");

			if (this._isPathDir(path2)) {
				if (this.fs.has(path2.slice(0, -1))) return this._setError("Rename failed: File collision");
			} else {
				if (this.fs.has(path2 + "/")) return this._setError("Rename failed: Directory collision");
			}

			if (!this.hasPermission(path2, "create")) return this._setError("Rename failed: No 'create' permission");
		...
		copy({
			...
			if (this.fs.has(path2)) return this._setError("Copy failed: Destination exists");
			if (!this.hasPermission(path2, "create")) return this._setError("Copy failed: No 'create' permission");
		...
		start({
			...
			if (this.fs.has(path)) return this._setError("Create failed: Path exists");

			if (this._isPathDir(path)) {
				if (this.fs.has(path.slice(0, -1))) return this._setError("Create failed: File collision");
			} else {
				if (this.fs.has(path + "/")) return this._setError("Create failed: Directory collision");
			}

			const parentDir = this._internalDirName(path);
			if (parentDir !== "/" && !this.fs.has(parentDir)) {
				if (!this.hasPermission(parentDir, "create")) return this._setError("Create failed: No permission on parent");
				this.start({
					STR: parentDir
				});
		...
		del({
			...
			const path = this._normalizePath(STR);
			if (!path) return this._setError("Invalid path");
			if (path === "/") return this._setError("Delete failed: Cannot delete root");
			...
			if (path.startsWith("/.Trash/")) {
				...
				this._ensureTrash();
				const name = path.endsWith("/") ? path.split("/").slice(-2, -1)[0] + "/" : path.split("/").pop();
		...
		runIntegrityTest() {
			...
			try {
				...
				if (!this.childIndex.get("/").has("/a.txt")) throw new Error("Index failed");
				this.del({
					STR: "/a.txt"
				});

				if (this.lastError) throw new Error("Delete op failed: " + this.lastError);
