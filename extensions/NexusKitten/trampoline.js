...
(function (Scratch) {
  ...
  class nktrampoline {
    ...
    async userFollowAll(args) {
      ...
      var list = [];
      while (true) {
        var userData = await Scratch.fetch(
          "https://corsproxy.io/?https://api.scratch.mit.edu/users/" +
            args.WHO +
            "/" +
            args.ATTRIBUTE +
            "?limit=40&offset=" +
    ...
    async userInfo(args) {
      const userData = await Scratch.fetch(
