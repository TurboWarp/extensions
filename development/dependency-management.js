...
const isImmutableURL = (url) => (
  ...
  /^https:\/\/cdn\.jsdelivr\.net\/npm\/(?:[\w-]+|@[\w-]+\/[\w-]+)@\d+\.\d+\.\d+[^\/]*\//.test(url) ||
  ...
  /^https:\/\/raw\.githubusercontent\.com\/[\w-]+\/[\w-]+\/[a-f0-9]{40}\//.test(url)
);
...
export const validateImports = (js) => {
  ...
  for (const i of imports) {
    if (!isImmutableURL(i.url)) {
      throw new Error(`${i.url} is not a known immutable URL. npm packages need to be pinned to an exact version. Git repositories need to be pinned to an exact commit hash. Extend isImmutableURL() in development/dependency-management.js if this is false positive.`);
    }
  }
};
