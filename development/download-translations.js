...
const removeUnchangedTranslations = (source, translated) => {
    ...
    for (const [key, translatedValue] of Object.entries(translated)) {
        ...
        if (typeof translatedValue === 'object') {
            const recursiveResult = removeUnchangedTranslations(sourceValue, translatedValue);
...
const downloadTranslatedResource = async (resource, locale) => {
    ...
    if (locale === SOURCE_LOCALE) {
        ...
        urlToDownload = await transifexApi.ResourceTranslationsAsyncDownload.download({
            mode: 'onlytranslated',
            resource: {
                data: {
                    id: `o:${ORGANIZATION_NAME}:p:${PROJECT_NAME}:r:${resource}`,
    ...
    const withoutEmptyTranslations = removeEmptyTranslations(withoutDeveloperComments);
...
const downloadAllResourceTranslations = async (resource) => {
    ...
    for (const [locale, translatedStringCount] of Object.entries(transifexStatistics)) {
        if (translatedStringCount > 0) {
            localesToFetch.push(locale);
        }
    }

    const entries = await Promise.all(localesToFetch.map(async (locale) => {
        const translatedStrings = await downloadTranslatedResource(resource, locale);
    ...
    for (const [locale, strings] of entries) {
        if (locale !== SOURCE_LOCALE) {
            const withoutUnchangedStrings = removeUnchangedTranslations(sourceStrings, strings);
...
const run = async () => {
    ...
    fs.writeFileSync(pathUtil.join(__dirname, '../translations/extension-runtime.json'), JSON.stringify(runtimeStrings, null, 4));
    fs.writeFileSync(pathUtil.join(__dirname, '../translations/extension-metadata.json'), JSON.stringify(metadataStrings, null, 4));
};
