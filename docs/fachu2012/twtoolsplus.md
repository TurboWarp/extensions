...
**Version:** 1.4.4
**Extensions:** Tools+ Math, Tools+ Text, Tools+ Data, Tools+ Sprite, Tools+ Time, Tools+ Bullets, Tools+ Extra
**Extension IDs:** `twtoolsplusmath`, `twtoolsplustext`, `twtoolsplusdata`, `twtoolsplussprite`, `twtoolsplustime`, `twtoolsplusbullets`, `twtoolsplusextra`
**Requires:** TurboWarp, **Run without sandbox**

This document describes every block in the full **TW Tools+** pack.

The pack is divided into seven extensions. Lists, dictionaries, and many data tools use **JSON text** (e.g. `["a","b"]`, `{"k":1}`), not Scratch's built-in list UI.

## Packs

| Pack in palette | Extension ID         | Sections inside               |
| --------------- | -------------------- | ----------------------------- |
| Tools+ Math     | `twtoolsplusmath`    | Math, Geometry                |
| Tools+ Text     | `twtoolsplustext`    | Text, Lists                   |
| Tools+ Data     | `twtoolsplusdata`    | JSON, Dictionaries, Storage   |
| Tools+ Sprite   | `twtoolsplussprite`  | Sprite / Stage, Camera, Input |
| Tools+ Time     | `twtoolsplustime`    | Time, Control flow, Debug     |
| Tools+ Bullets  | `twtoolsplusbullets` | Bullets                       |
| Tools+ Extra    | `twtoolsplusextra`   | Color, Audio, Network, Misc   |
...
* Some Tools+ features use TurboWarp VM or browser APIs directly and therefore require **Run without sandbox**.
